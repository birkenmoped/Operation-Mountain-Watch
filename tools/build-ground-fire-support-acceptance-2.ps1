...
$requiredMarkers = @(
  ...
  'ARTY:New(','AssignTargetCoord(','GetAmmo(','SetRearmingGroup','SetSpawnZone(','SetValidateAndRepositionGroundUnits','ReturnToStock','AddAsset(',
...
if ($fixedSupport -notmatch 'brigade:SetValidateAndRepositionGroundUnits\(true\)') { throw 'Fixed fire-support support module must enable public MOOSE ground reposition validation.' }
...
-- Gate/Test-ID: $testId
-- Scope: concurrent Bostick/Wright/Fortress L118 plus Honaker 2B11 firing -> local M1083 Warehouse self-request materialization in dedicated RESUPPLY zones -> local CampaignState GROUND_AMMO_PACKAGE consumption -> MOOSE ARTY RearmingGroup rearm -> MOOSE ARTY physical support return -> WAREHOUSE AddAsset return-to-stock -> per-site and aggregate confirmation.
