...
function TM02W2FTransitRepresentation.install(config, executionState, navigation)
  ...
  local function nextAlias(task, prefixValue)
    ...
    return prefixValue .. task.taskId:gsub("[^%w]", "_")
      .. "_M" .. string.format("%04d", generation)
  end
  ...
  local function syncExpandedSurvivors(task, group)
    ...
    if alive < previous then
      ...
      log("WARNING", "expanded_group_losses_recorded_before_conversion", {
        taskId = task.taskId,
  ...
  local function convertTask(task, unpack, reason)
    if task.movementState ~= "EN_ROUTE" or not task.proxyGroup
      or task.proxyGroup:IsAlive() ~= true then
      return false, "TASK_NOT_TRAVELLING"
    end
    local oldGroup = task.proxyGroup
    if not syncExpandedSurvivors(task, oldGroup) then
  ...
  local function beginSerializedConversion(unpack)
    ...
    if #candidates == 0 then
      announce(unpack and "Keine gepackten Reise-Proxies vorhanden"
        or "Keine entpackten Reisegruppen vorhanden")
      return true
    end
    ...
    announce((unpack and "Reise-Proxies werden entpackt: "
      or "Reisegruppen werden gepackt: ") .. #candidates)
    ...
    timer.scheduleFunction(function()
      ...
      if not task then
        ...
        announce((unpack and "Reise-Proxies entpackt: " or "Reisegruppen gepackt: ")
          .. converted .. " / " .. #candidates .. "; uebersprungen: "
          .. skipped .. "; Fehler: " .. errors)
        return nil
      end
      ...
      if not stillEligible then
        ...
        local ok, success, failure = pcall(
          convertTask,
          task,
          unpack,
        ...
        if ok and success == true then
          ...
          log("ERROR", "transit_conversion_failed", {
            taskId = task.taskId,
            operation = operation,
            reason = ok and tostring(failure) or tostring(success),
          })
        end
      end
      index = index + 1
  ...
  local function showStatus()
    ...
    log("INFO", "transit_representation_status", {
      ...
      physicalMode = "DIRECT_OFFROAD",
      automaticRecoveryInterface = false,
    })
  ...
  if state.valid and config.transitRepresentation.enableF10Menu == true then
    ...
    MENU_MISSION_COMMAND:New(config.transitRepresentation.unpackCommand, menu,
      function() return beginSerializedConversion(true) end)
    MENU_MISSION_COMMAND:New(config.transitRepresentation.packCommand, menu,
      function() return beginSerializedConversion(false) end)
    ...
    if type(executionState.showCommanderStatus) == "function" then
      MENU_MISSION_COMMAND:New(config.transitRepresentation.commanderStatusCommand,
        menu, executionState.showCommanderStatus)
    end
    MENU_MISSION_COMMAND:New(config.transitRepresentation.listCommand,
      menu, executionState.showTasks)
    MENU_MISSION_COMMAND:New(config.transitRepresentation.markerCommand,
      menu, executionState.toggleMarkers)
