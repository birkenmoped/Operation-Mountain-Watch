-- Operation Mountain Watch - Stage 1D-S Ground SUPPLY RESUPPLY acceptance.
-- Test-ID: GROUND-SUPPLY-RESUPPLY-NOTHING-ACCEPTANCE-1
--
-- Acceptance-only vertical slice:
-- Honaker CampaignState SUPPLY shortage -> ResourceDemandPolicy -> MissionDemand
...
local TEST_ID = "GROUND-SUPPLY-RESUPPLY-NOTHING-ACCEPTANCE-1"
local TAG = "[OMW][" .. TEST_ID .. "]"

local ORIGIN_NODE = "GROUND_NODE_JOYCE"
local DESTINATION_NODE = "GROUND_NODE_HONAKER"
local RESOURCE_ID = "GROUND_SUPPLY_PACKAGE"
local RESOURCE_CLASS = "GROUND_SUPPLY"
...
local BRIGADE_NAME = "BDE_BLUE_GND_JOYCE_SUPPLY_RESUPPLY_ACCEPTANCE"
local PLATOON_NAME = "PLT_BLUE_GND_JOYCE_SUPPLY_RESUPPLY_ACCEPTANCE"
local DEMAND_ID = "RESUPPLY-ACCEPTANCE-HONAKER-SUPPLY-001"
local TRANSFER_ID = "TRANSFER-ACCEPTANCE-JOYCE-HONAKER-SUPPLY-001"
local SHORTAGE_TX_ID = "CONSUMPTION-ACCEPTANCE-HONAKER-SUPPLY-001"
local CARRIER_ENTITY_ID = "GROUND-RESUPPLY-JOYCE-HONAKER-SUPPLY-CONVOY-LIGHT-001"
local ASSIGNEE_ID = "AI:BRIGADE:" .. BRIGADE_NAME

local INITIAL_ORIGIN = 48
local INITIAL_DESTINATION = 40
local SHORTAGE_QUANTITY = 20
local SHORTAGE_DESTINATION = 20
local TRANSFER_QUANTITY = 20
local FINAL_ORIGIN = 28
...
local function attachArmyGroupCallbacks(armyGroup)
  if armyGroup.__omwGroundSupplyResupplyNothingAcceptanceCallbacks then return end
  armyGroup.__omwGroundSupplyResupplyNothingAcceptanceCallbacks = true
...
local function installBrigadeCallbacks()
  ...
  state.brigade.OnAfterArmyOnMission = function(self, From, Event, To, ArmyGroup, Mission)
    ...
    log("ARMY_ON_MISSION group=" .. tostring(ArmyGroup:GetName())
      .. " mission=NOTHING source=AUFTRAG_NEW_NOTHING"
...
local function createDemandAndReservation()
  ...
  if not expectEqual(state.candidate.resourceClass, RESOURCE_CLASS, "CANDIDATE_RESOURCE_CLASS") then return false end
  ...
  local demand, demandCreated = state.registry:Create({
    ...
    origin = ORIGIN_NODE,
    objective = "Restore Honaker Ground SUPPLY to target stock through one-shot MOOSE NOTHING",
...
local function preparePhysicalExecution()
  ...
  log("PHYSICAL_EXECUTION_READY warehouse=" .. WAREHOUSE_NAME
    ...
    .. " missionCreation=AUFTRAG_NEW_NOTHING"
...
if not state.stockRow then fail("HONAKER_SUPPLY_STOCK_ROW_NOT_FOUND"); return end
if not expectEqual(state.stockRow.resourceClass, RESOURCE_CLASS, "STOCK_ROW_RESOURCE_CLASS") then return end
