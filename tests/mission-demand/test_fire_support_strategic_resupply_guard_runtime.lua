...
local prepared,installedMaterializer={},{}
local Materializer={}
...
for siteId,site in pairs(Sites.Sites) do
  ...
  eq(resolvedPathlines[siteId],site.guardRoute.pathlineName,siteId.." owner PATHLINE resolved for compact placement")
  ...
  eq(runtime:GetRouteAdapter(siteId),nil,siteId.." no production Guard patrol router")
end
eq(count,6,"six sites assembled")

local demand={demandId="INCIDENT|FOB_JOYCE|GUARD|INSTALLATION_ATTACK_LOCAL_GUARD",siteId="FOB_JOYCE",supportType="GUARD",priority=40}
...
eq(handle.mission.returnToLegion,true,"Guard uses MOOSE return-to-Legion lifecycle")
...
-- The accepted PATHLINE materializer is retained only for compact local placement.
-- No continuous PATHLINE patrol is installed or tracked by production GuardRuntime.
