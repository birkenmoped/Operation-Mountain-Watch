...
foreach ($requiredFile in @($foundationSource, $acceptanceSource, $validatedCsv, $lifecycleGuard)) {
    if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
        throw "Required final acceptance input not found: $requiredFile"
    }
}
...
$combinedSource = $foundation + "`n`n-- === FINAL PARKING ACCEPTANCE HARNESS ===`n`n" + $acceptance
...
foreach ($pattern in $forbiddenPatterns) {
    if ($combinedSource -match $pattern) {
        throw "Final parking acceptance contains forbidden non-MOOSE/override path: $pattern"
    }
}
...
-- Scope: Bagram foundation + 187/187 parking baseline + 69/69 parking propagation + seven controlled MOOSE ALERT5 materializations + physical TerminalID verification.
...
Write-Host "Scope: BAGRAM_PARKING_FINAL_ACCEPTANCE"
