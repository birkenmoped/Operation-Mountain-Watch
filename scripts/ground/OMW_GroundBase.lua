...
-- and runtime integration modules remain the accepted implementation sources.

local GroundBase = {}

local TAG = "[OMW][Ground.Base]"

GroundBase.SchemaVersion = "OMW-GROUND-PRODUCTION-BASE-1"
...
function GroundBase.Attach(spec)
  ...
  if not GroundInitialStock or not GroundCampaignStateAdapter or not GroundRuntimeIntegration then
...
function GroundBase.GetConfig()
  return {
    ...
    configured = GroundInitialStock ~= nil and GroundCampaignStateAdapter ~= nil and GroundRuntimeIntegration ~= nil,
