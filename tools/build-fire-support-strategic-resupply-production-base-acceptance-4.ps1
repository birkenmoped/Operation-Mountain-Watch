...
if($p.Contains('local DEFAULT_ENGAGE_FORMATION = "Vee"')){throw 'Production QRF MissionFactory must not default to Vee.'}
if($p.Contains('local QRF_ENGAGE_FORMATION = "Vee"')){throw 'Production QRF runtime must not override motorized march with Vee.'}
...
Write-Host 'QrfMovementContract: motorized QRF runtime explicitly passes MOOSE On Road to EngageTarget; Vee runtime override is forbidden'
