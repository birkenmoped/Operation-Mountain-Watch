...
$sourceFile = Join-Path $repoRoot 'mission\tests\army-ground-foundation\src\02-army-ground-acceptance-2.lua'
$distDir = Join-Path $repoRoot 'mission\tests\army-ground-foundation\dist'
$outputFile = Join-Path $distDir 'OMW_Army_Ground_Acceptance_2.lua'

$builderVersion = 'ARMY-GROUND-ACCEPTANCE-2-1'
$testId = 'ARMY-GROUND-ACCEPTANCE-2-1'
...
$requiredMarkers = @(
  ...
  'PLATOON:New(',
  'platoon:AddMissionCapability(AUFTRAG.Type.ARMOREDGUARD',
  ...
  'function armyGroup:OnAfterMissionExecute',
  'function armyGroup:OnAfterMissionDone',
  'platoon:CountAssets(true, AUFTRAG.Type.ARMOREDGUARD)',
  'OMW_GND_A2',
  'GROUP_MATERIALIZED',
  'APPROACH_GUARD_EXECUTING',
  'OBS_GUARD_EXECUTING',
  'GUARD_HOLD_STABLE',
...
-- Builder: tools/build-army-ground-acceptance-2.ps1
-- BuilderVersion: $builderVersion
-- GitCommit: $commit
-- GeneratedUtc: $generatedUtc
-- Gate/Test-ID: $testId
-- Scope: Joyce mounted security test; ARMOREDGUARD On Road approach; same-group Vee tactical leg; ARMOREDGUARD full-stop observation halt; SetReturnToLegion(false); runtime hold-stability check.
-- Exclusions: no per-vehicle sector assignment; no full CampaignState adapter; no restart/reconstitution; no Returned->Warehouse acceptance; no OPSTRANSPORT; no QRF; no artillery; no combat-loss settlement; no MIZ mutation.
...
Write-Host "ObservationZone: ZON_BLUE_GND_JOYCE_PATROL_TEST_01"
Write-Host "Mission1: ARMOREDGUARD / On Road / 10 kt"
