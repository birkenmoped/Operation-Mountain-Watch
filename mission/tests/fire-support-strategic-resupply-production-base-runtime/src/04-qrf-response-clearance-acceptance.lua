...
-- Focused Joyce regression for the owner-approved QRF response -> clearance lifecycle.
--
...
--
-- The harness does not implement target search, does not use GROUNDATTACK and does
-- not create Mission Editor zones. Its scheduler is observation/test stimulus only.
...
local function routeFixture()
  if state.released then return true end
  ...
  announce("ARMED","Joyce RED fixture physically routed into the owner-defined runtime alarm perimeter",15)
  return true,nil
end
...
local function observe()
  ...
  if not state.released and state.guardMove>=MIN_GUARD_MOVE_M then
    local ok,reason=routeFixture()
  ...
  if state.startedAt and timer.getTime()-state.startedAt>TEST_TIMEOUT_SEC then fail("TIMEOUT_INCOMPLETE_QRF_RESPONSE_CLEARANCE_CHAIN") end
end
...
local function start()
  ...
  local _,started,perimeterReason=runtime:StartPerimeters()
  if started~=true then fail("PERIMETER_START_FAILED "..tostring(perimeterReason)); return end
  state.brigade:Start()
  local _,created,siteReason=runtime:StartSite(SITE_ID,{})
  if created==false then fail("GUARD_START_FAILED "..tostring(siteReason)); return end

  state.startedAt=timer.getTime()
  announce("READY","Joyce focused QRF response-clearance acceptance armed; fixture remains late-activated until Guard moves >=25 m",20)
