...
The current design corrects both problems and the later singleton defect:
...
5. the controller uses `state.packets` and `state.packetById`, never one shared `state.packet`.
...
A successful version 3 bootstrap installs the full global menu plus one independent submenu for each configured packet. The former singleton design is superseded and must not recur.
