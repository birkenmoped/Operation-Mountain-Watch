...
local state = {
  ...
  casAdapter=nil, casMission=nil, casExecuting=false, casCorridor=false,
  battery=nil, arty=nil, fireAdapter=nil, fireDemand=nil, fireStarted=false, fireComplete=false,
  physicalAmmoBefore=nil, physicalAmmoAfter=nil, rearmService=nil, rearmComplete=false, supportReturned=false,
...
local function context()
  ...
  if type(OMW) ~= "table" or type(OMW.Ground) ~= "table" or type(OMW.Ground.Base) ~= "table" then
    fail("OMW Ground Base unavailable") return nil
  end
...
local function startAirResupply()
  ...
  local transfer, transferCreated = ctx.store:ReserveResource({ transactionId=TRANSFER_ID, reservationId="MISSION-DEMAND:"..RESUPPLY_DEMAND_ID,
    cargoId=CARGO_ID, missionDemandId=RESUPPLY_DEMAND_ID, carrierEntityId=CARRIER_ID,
    kind=ctx.campaignState.TransactionKind.TRANSFER, resourceId=AMMO_RESOURCE, quantity=15, canonicalUnit="count",
    originNodeId=JALALABAD_NODE, destinationNodeId=WRIGHT_NODE })
  if transferCreated ~= true then fail("Air-AMMO transfer reservation failed") return end
  registry:SetReservationState(RESUPPLY_DEMAND_ID, "RESERVED", { transactionId=TRANSFER_ID, cargoId=CARGO_ID,
    originNodeId=JALALABAD_NODE, destinationNodeId=WRIGHT_NODE, resourceId=AMMO_RESOURCE, quantity=15, carrierEntityId=CARRIER_ID })
...
local function dispatchFire(opsZone,incident)
  local targets = redGroups(opsZone); if #targets==0 then fail("no RED target for Wright fire support") return end
  local target=targets[1]; local p=target:GetCoordinate():GetVec3()
  local demand,created,reason = FirePolicy.CreateDemand(MissionDemand,registry,incident,{ targetKind="DETECTED_RED_GROUND_GROUP",targetName=target:GetName(),position={x=p.x,y=p.y,z=p.z} })
  if created~=true then fail("fire-support demand failed: "..tostring(reason)) return end
  state.fireDemand=demand
  msg("FIRE SUPPORT","Honaker requests immediate fire support; local mortar unavailable",12)
  msg("FIRE SUPPORT","Wright L118 selected and assigned",9)
  local _,dispatched,dispatchReason=state.fireAdapter:Dispatch(demand,target); if dispatched~=true then fail("Wright ARTY dispatch failed: "..tostring(dispatchReason)) end
end
  ...
  state.brigade.OnAfterStart=function()
    SCHEDULER:New(nil,function()
      ...
      state.threat=ThreatAdapter.New({ missionDemand=MissionDemand,registry=registry,policy={CreateDemand=function(md,reg,incident)
        ...
        if created then local mission,ok,why=state.casAdapter:Dispatch(demand,state.threat.securityZone); if ok then state.casMission=mission; local prev=mission.OnAfterExecuting; function mission:OnAfterExecuting(F,E,T) if prev then prev(self,F,E,T) end; state.casExecuting=true; msg("CAS","Jalalabad AH-64D CAS executing for Honaker",9) end else fail("CAS dispatch failed: "..tostring(why)) end end
        ...
        onThreatStarted=function(_,opsZone,demand,created,reason,incident) if state.threatStarted then return end; state.threatStarted=true; state.incident=incident; msg("THREAT","COP Honaker under attack - MOOSE OPSZONE Attacked confirmed",12); dispatchQrf(opsZone); dispatchFire(opsZone,incident) end })
      state.threat:Start(); msg("READY","Honaker full-response acceptance armed: QRF + CAS + Wright L118 + rearm + CH-47 Air-AMMO",15)
    end,{},5)
  end
  state.brigade:Start()
end
...
local function start()
  ...
  state.casAdapter=CasAdapter.New({missionDemand=MissionDemand,registry=registry,airwing=state.airwing,assigneeId="AIRWING:AW_US_JBAD_TF_SHOOTER_6_6_CAV",casAltitudeFt=10000,casSpeedKts=120})
