...
$acceptanceSourceFile = Join-Path $repoRoot 'mission\tests\ground-resupply-execution\src\03-ground-meta-resupply-nothing-acceptance.lua'
...
foreach ($file in $files) {
  if (-not (Test-Path -LiteralPath $file -PathType Leaf)) {
    throw "Required Ground meta RESUPPLY acceptance source not found: $file"
  }
}
...
foreach ($marker in $requiredMarkers) {
  if (-not $combined.Contains($marker)) {
    throw "Ground meta RESUPPLY acceptance sources are missing required marker: $marker"
  }
}
...
foreach ($pattern in $forbiddenPatterns) {
  if ($acceptanceSource -match $pattern) {
    throw "Ground meta RESUPPLY acceptance contains forbidden runtime pattern: $pattern"
  }
}
...
if ([string]::IsNullOrWhiteSpace($commit)) {
  throw 'Unable to resolve Git HEAD for Ground meta RESUPPLY acceptance build.'
}
$generatedUtc = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')

$header = @"
-- AUTO-GENERATED FILE. DO NOT EDIT DIRECTLY.
-- Builder: tools/build-ground-meta-resupply-nothing-acceptance-1.ps1
-- BuilderVersion: $builderVersion
-- GitCommit: $commit
-- GeneratedUtc: $generatedUtc
-- TestId: $testId
-- Scope: Joyce -> Honaker CampaignState meta-resource shortage / MissionDemand / neutral MOOSE AUFTRAG NOTHING movement / same-group return acceptance.
...
Write-Host 'StrategicResourceModel: CampaignState meta resource'
...
Write-Host 'OutboundTimeoutSec: 600'
