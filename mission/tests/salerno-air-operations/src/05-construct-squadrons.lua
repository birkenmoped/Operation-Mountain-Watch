...
local function main()
  ...
  if not SQUADRON then
    log("COMPLETE status=FAIL reason=moose-squadron-class-unavailable")
    return
  end
  ...
  for _, contract in ipairs(contracts) do
    ...
    log(string.format(
      "CONTRACT name=%s template=%s logicalAircraft=%d unitsPerTemplate=%d ngroups=%d representedAircraft=%d residualAircraft=%d",
    ...
    if expectedResidual ~= contract.ResidualAircraft or expectedResidual < 0 then
      ...
      if ok and squadronOrError then
        ...
        log("CONSTRUCTED name=" .. contract.Name .. " template=" .. contract.Template .. " ngroups=" .. tostring(contract.Ngroups))
      else
        failures = failures + 1
  ...
  OMW.AirOps.SalernoDiagnostics.ConstructedSquadrons = constructed

  log(string.format(
    "SAFETY airwingStartCalled=false squadronsAddedToAirwing=0 missionsAdded=0 spawnsExpected=0 residualAircraft=%d",
    residualAircraft))

  local pass = failures == 0 and #constructed == cfg.Expected.Squadrons
  log(string.format("SUMMARY constructed=%d/%d failures=%d residualAircraft=%d",
    #constructed, cfg.Expected.Squadrons, failures, residualAircraft))
