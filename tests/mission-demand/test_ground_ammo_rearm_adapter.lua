...
local function newFakeArty(options)
  ...
  function arty:SetRearmingDistance(distance)
    self.rearmingDistance = distance
    return self
  end

  function arty:SetRearmingGroupSpeed(speed)
    self.rearmingSpeedKph = speed
    return self
  end
  ...
  function arty:CompleteRearm()
    if self.OnAfterRearmed then
      self:OnAfterRearmed({}, "Rearming", "Rearmed", "Rearmed")
    end
  end
...
-- Successful local rearm: reserve first, consume in ARTY OnBeforeRearm, then
-- complete through the public OnAfterRearmed hook.
do
...
-- If ARTY rejects the rearm before OnBeforeRearm, the strategic reservation is
-- cancelled and no package is consumed.
do
  local store = newStore(3)
...
-- Repeating the same request is idempotent at the adapter layer.
do
  local store = newStore(3)
  local fakeArty = newFakeArty()
