...
local function makeDemand(registry, id, missionType, aiCapable)
  local demand = registry:Create({
    ...
    origin = "BLUE_GROUND_COP_FORTRESS",
    objective = "FOB_ATTACK_SUPPORT",
    target = { installationId = "BLUE_GROUND_COP_FORTRESS" },
    priority = 90,
    playerCapable = true,
    aiCapable = aiCapable,
    dedupeKey = missionType .. "|" .. id,
  })
  return demand
end
...
local added = {}
local airwing = {
  AddMission = function(self, mission)
    added[#added + 1] = mission
    return self
  end,
  I = function() end,
}
...
assertEqual(registry:Get(demand.id).assignedTo, "AIRWING:AW_US_JBAD_TF_SHOOTER_6_6_CAV", "assignedTo")
...
assertEqual(factoryCalls, 1, "duplicate factory calls")
assertEqual(#added, 1, "duplicate airwing missions")
...
local failedAdapter = DispatchAdapter.New({
  ...
  auftragFactory = function()
    return { GetName = function() return "TEST_FAILED" end }
  end,
})
