...
-- coordinate fire missions -> physically verified fire -> real local M1083 rearm
...
local state = {
  ...
  threat=nil, threatStarted=false, incident=nil,
  ...
  battery=nil, arty=nil, fireAdapter=nil, fireDemand=nil, fireStarted=false, fireComplete=false, fireTargetCount=0, fireTargetCompleteCount=0,
...
local function stockRow(nodeId, resourceId)
  for _, row in ipairs(OMW.Ground.Base.GetInitialStock().Rows or {}) do if row.nodeId == nodeId and row.resourceId == resourceId then return row end end
  return nil
end
local function redGroups(opsZone)
  ...
  table.sort(result, function(a,b) return state.guardCoord:Get2DDistance(a:GetCoordinate()) < state.guardCoord:Get2DDistance(b:GetCoordinate()) end)
  return result
end
  ...
  local function attempt()
    ...
    if #pathlineNames == 1 then
      ...
      resolved = HelicopterCorridor.ResolveSequence({
        pathlineNames=pathlineNames, originCoordinate=flight:GetCoordinate(), destinationCoordinate=destination,
    ...
    if ok then
      ...
      local profileText = kind == "CAS" and string.format("; WEST %d ft AGL / RotaryWing.Column.D70", WEST_ALTITUDE_FT_AGL) or ""
...
local function prepareAirwing()
  ...
  state.airwing = air.Airwing; state.ch47 = air.Squadrons.CH47
  need(GROUP:FindByName("TPL_AIR_US_JBAD_CH47_HEAVYLIFT_1SHIP"), "Jalalabad CH47 template")
  need(PATHLINE:FindByName(PRIMARY_PATHLINE), PRIMARY_PATHLINE); need(PATHLINE:FindByName(WEST_PATHLINE), WEST_PATHLINE)
...
local function installAirObserver()
  ...
  function state.airwing:OnAfterFlightOnMission(From, Event, To, FlightGroup, Mission)
    ...
    if Mission == state.casMission then
      ...
      msg("CAS", "Jalalabad AH-64D assigned to Honaker; real weapon employment required", 10)
    ...
    state.airFlight = FlightGroup; state.airAsset = Mission:GetAssetByName(FlightGroup:GetName())
    if not state.airAsset then fail("CH47 mission asset not found") return end
    local tx = context().store:MarkLoading(TRANSFER_ID); registry:SetReservationState(RESUPPLY_DEMAND_ID, "LOADING")
    ...
    function FlightGroup:OnAfterLanded(F,E,T,Airbase)
      ...
      if state.delivered and Airbase and Airbase:GetName() == state.airwing:GetAirbaseName() then state.homeLanded = true; msg("LOGISTICS", "CH-47 landed back at Jalalabad via OMW_FlightPath", 9) end
    end
    state.transitScheduler = SCHEDULER:New(nil, function()
      ...
      context().store:MarkInTransit(TRANSFER_ID); registry:SetReservationState(RESUPPLY_DEMAND_ID, "IN_TRANSIT")
      local demand = registry:Get(RESUPPLY_DEMAND_ID); if demand and demand.status == MissionDemand.Status.AI_ASSIGNED then registry:Activate(RESUPPLY_DEMAND_ID) end
      state.inTransit = true; msg("LOGISTICS", "Air-AMMO cargo picked up; Jalalabad -> Wright IN TRANSIT", 10)
    end, {}, 5, 5)
  end
  ...
  function state.airwing:OnAfterLegionAssetReturned(From, Event, To, Cohort, Asset)
    ...
    if state.airAsset and Asset == state.airAsset then
      ...
      state.assetReturned = true; msg("LOGISTICS", "CH-47 recovered by Jalalabad AIRWING", 8)
    end
  end
end

local function preconditionWright()
  local ctx = context(); local before = ctx.store:GetResource(WRIGHT_NODE, AMMO_RESOURCE)
...
local function startAirResupply()
  ...
  local ctx = context(); local row = stockRow(WRIGHT_NODE, AMMO_RESOURCE); local wright = ctx.store:GetResource(WRIGHT_NODE, AMMO_RESOURCE)
  ...
  local demand, created, reason = ResourceDemandCoordinator.EvaluateAndCreate({ policy=ResourceDemandPolicy, missionDemand=MissionDemand,
    registry=registry, store=ctx.store, row=row, demandIdFactory=function() return RESUPPLY_DEMAND_ID end })
  if not demand or created ~= true then fail("Wright RESUPPLY demand failed: " .. tostring(reason)) return end
  state.resupply = demand
  local duplicate, duplicateCreated, duplicateReason = ResourceDemandCoordinator.EvaluateAndCreate({ policy=ResourceDemandPolicy,
    missionDemand=MissionDemand, registry=registry, store=ctx.store, row=row, demandIdFactory=function() return "DUPLICATE" end })
  ...
  local transfer, transferCreated = ctx.store:ReserveResource({ transactionId=TRANSFER_ID, reservationId="MISSION-DEMAND:"..RESUPPLY_DEMAND_ID,
    cargoId=CARGO_ID, missionDemandId=RESUPPLY_DEMAND_ID, carrierEntityId=CARRIER_ID, kind=ctx.campaignState.TransactionKind.TRANSFER,
  ...
  registry:SetReservationState(RESUPPLY_DEMAND_ID, "RESERVED", { transactionId=TRANSFER_ID, cargoId=CARGO_ID, originNodeId=JALALABAD_NODE,
    destinationNodeId=WRIGHT_NODE, resourceId=AMMO_RESOURCE, quantity=15, carrierEntityId=CARRIER_ID })
  local pickup = need(ZONE:FindByName(PICKUP_ZONE), PICKUP_ZONE); local drop = need(ZONE:FindByName(DROP_ZONE), DROP_ZONE); if state.failed then return end
  ...
  state.airMission = AUFTRAG:NewCARGOTRANSPORT(state.cargo, drop); state.airMission:SetName("OMW_STAGE3_E2E_AIR_AMMO_JALALABAD_TO_WRIGHT")
  state.airMission:SetRequiredAssets(1,1); state.airMission:AssignSquadrons({state.ch47}); state.airMission:SetPriority(20,true)
  ...
  function state.airMission:OnAfterSuccess(From,Event,To)
    ...
    state.delivered = true; msg("LOGISTICS", "Air-AMMO delivered at Wright; strategic stock restored to 30 / 30", 12)
  end
  local oldFailed = state.airMission.OnAfterFailed
  function state.airMission:OnAfterFailed(From,Event,To) if oldFailed then oldFailed(self,From,Event,To) end; fail("MOOSE CARGOTRANSPORT failed") end
  registry:AssignAI(RESUPPLY_DEMAND_ID,"AI:SQUADRON:SQ_US_JBAD_CH47_HEAVYLIFT"); state.airwing:AddMission(state.airMission)
...
local function setupFireSupport()
  ...
  state.arty = ARTY:New(state.battery,"Wright L118 Stage3 E2E"); state.arty:SetReportOFF(); state.arty:SetWaitForShotTime(ARTY_WAIT_FOR_SHOT_SEC); state.arty:Start()
  ...
  state.rearmService = FixedFireSupportAmmoRearmService.New({ fixedFireSupportAmmoSupportModule=FixedFireSupportAmmoSupport,
    ...
    platoonFactory=function(t,c,n) return PLATOON:New(t,c,n) end, descriptorGroupName=WAREHOUSE.Descriptor.GROUPNAME,
    templateName=M1083_TEMPLATE, platoonName="PLT_BLUE_GND_WRIGHT_STAGE3_E2E_REARM", assignment="OMW:WRIGHT:AMMO-SUPPORT:STAGE3-E2E",
    carrierEntityId="WRIGHT-AMMO-SUPPORT-M1083-STAGE3-E2E", nodeId=WRIGHT_NODE, alias="Wright L118 Stage3 E2E", stockCount=1, priority=20,
    returnCheckIntervalSec=5, returnTimeoutSec=300, log=function(level,text) log("REARM "..tostring(level).." "..tostring(text)) end,
  ...
  state.fireAdapter = FireAdapter.New({ missionDemand=MissionDemand, registry=registry, arty=state.arty, assigneeId="ARTY:WRIGHT:L118",
    ...
    onFireStarted=function(_,target)
      ...
      msg("FIRE SUPPORT",string.format("Wright L118 firing at %s; physical ammo before=%s",targetName,tostring(ammo)),12)
    end,
    verifyFireComplete=function(_,target)
      ...
      state.physicalAmmoAfterByTarget[targetName] = after; state.physicalAmmoAfter = after
    ...
    onTargetComplete=function(_,target)
      state.fireTargetCompleteCount = state.fireTargetCompleteCount + 1
      local targetName = target and target.name or "unknown"
      msg("FIRE SUPPORT",string.format("Wright target %d/%d complete: %s ammo %s -> %s",state.fireTargetCompleteCount,state.fireTargetCount,targetName,
        tostring(state.physicalAmmoBeforeByTarget[targetName]),tostring(state.physicalAmmoAfterByTarget[targetName])),12)
    end,
    onFireRejected=function(_,target,_,reason)
      local targetName = target and target.name or "unknown"
      msg("FIRE SUPPORT",string.format("Wright physical fire NOT confirmed for %s: ammo %s -> %s; %s",targetName,
        tostring(state.physicalAmmoBeforeByTarget[targetName]),tostring(state.physicalAmmoAfterByTarget[targetName]),tostring(reason)),20)
    ...
    onFireComplete=function(demandId)
      ...
      msg("FIRE SUPPORT",string.format("Wright completed %d coordinate fire missions; total physical ammo %s -> %s; local M1083 rearm requested",
...
local function dispatchQrf(opsZone)
  local targets = redGroups(opsZone); if #targets == 0 then fail("no RED groups available for Honaker QRF") return end
  ...
  for i=1,count do
    local deployment = PersonnelLedger.New({ store=context().store,campaignState=context().campaignState,nodeId=HONAKER_NODE,resourceId=PERSONNEL_RESOURCE,
      deploymentId="STAGE3-HONAKER-QRF-"..i,entityId="HONAKER-QRF-"..i,quantity=QRF_PERSONNEL,missionDemandId=TEST_ID })
    local mission = AUFTRAG:NewGROUNDATTACK(targets[i],8,ENUMS.Formation.Vehicle.OffRoad); mission:SetName("OMW_STAGE3_HONAKER_QRF_"..i)
    local entry={mission=mission,target=targets[i],deployment=deployment,army=nil,engaged=false}; state.qrfEntries[#state.qrfEntries+1]=entry; state.brigade:AddMission(mission)
  end
end

local function dispatchFire(opsZone,incident)
  local detected = redGroups(opsZone); if #detected==0 then fail("no RED target for Wright fire support") return end
  local targets = {}; for i=1,math.min(#detected,FIRE_TARGETS_MAX) do targets[#targets+1]=detected[i] end
  local firstCoord = targets[1]:GetCoordinate(); local p=firstCoord:GetVec3()
  local demand,created,reason = FirePolicy.CreateDemand(MissionDemand,registry,incident,{ targetKind="DETECTED_RED_GROUND_GROUP",targetName=targets[1]:GetName(),position={x=p.x,y=p.y,z=p.z} })
  if created~=true then fail("fire-support demand failed: "..tostring(reason)) return end
  state.fireDemand=demand; state.fireTargetCount=#targets
  msg("FIRE SUPPORT",string.format("Honaker requests immediate fire support; local mortar unavailable; %d OPSZONE targets selected",#targets),12)
  local batteryCoord = state.battery:GetCoordinate(); local minRange = state.arty.minrange; local maxRange = state.arty.maxrange
  for index,target in ipairs(targets) do
    local coordinate=target:GetCoordinate(); local vec3=coordinate:GetVec3(); local distance=batteryCoord:Get2DDistance(coordinate)
    msg("FIRE SUPPORT",string.format("Target %d/%d %s at x=%.0f z=%.0f; Wright range %.2f km; envelope %.2f-%.2f km",index,#targets,target:GetName(),vec3.x,vec3.z,
      distance/1000,(tonumber(minRange) or 0)/1000,(tonumber(maxRange) or 0)/1000),12)
    if type(minRange)=="number" and distance < minRange then fail("Wright target below MOOSE ARTY minimum range: "..target:GetName()); return end
    if type(maxRange)=="number" and distance > maxRange then fail("Wright target beyond MOOSE ARTY maximum range: "..target:GetName()); return end
  end
  msg("FIRE SUPPORT","Wright L118 selected; MOOSE coordinate Fire At Point missions queued",10)
  local _,dispatched,dispatchReason=state.fireAdapter:DispatchTargets(demand,targets)
  if dispatched~=true then fail("Wright ARTY dispatch failed: "..tostring(dispatchReason)) end
end

local function setupDefenceAndThreat()
  state.brigade=BRIGADE:New(HONAKER_WAREHOUSE,"BDE_BLUE_GND_HONAKER_STAGE3_E2E"); state.guardCoord=state.brigade:GetCoordinate()
  local guard=PLATOON:New(INF_TEMPLATE,1,"PLT_BLUE_GND_HONAKER_STAGE3_GUARD"); guard:AddMissionCapability(AUFTRAG.Type.ONGUARD,100); state.brigade:AddPlatoon(guard)
  state.qrfPlatoon=PLATOON:New(INF_TEMPLATE,QRF_GROUPS,"PLT_BLUE_GND_HONAKER_STAGE3_QRF"); state.qrfPlatoon:AddMissionCapability(AUFTRAG.Type.GROUNDATTACK,100); state.brigade:AddPlatoon(state.qrfPlatoon)
  state.brigade.OnAfterArmyOnMission=function(self,From,Event,To,ArmyGroup,Mission)
    for i,entry in ipairs(state.qrfEntries) do if entry.mission==Mission then entry.army=ArmyGroup; local old=ArmyGroup.OnAfterEngageTarget; function ArmyGroup:OnAfterEngageTarget(F,E,T,Target,Speed,Formation) if old then old(self,F,E,T,Target,Speed,Formation) end; entry.engaged=true; state.qrfEngaged=true; msg("QRF","Honaker QRF group "..i.." engaging "..entry.target:GetName(),8) end end end
  end
  state.brigade.OnAfterStart=function()
    SCHEDULER:New(nil,function()
      local guardMission=AUFTRAG:NewONGUARD(state.guardCoord); guardMission:SetEngageDetected(SECURITY_RADIUS_M/1852,{"Ground Units"}); guardMission:SetName("OMW_STAGE3_HONAKER_SENTRY"); state.brigade:AddMission(guardMission)
      state.threat=ThreatAdapter.New({ missionDemand=MissionDemand,registry=registry,policy={CreateDemand=function(md,reg,incident)
        ...
        if created then
          ...
          if ok then
            ...
            function mission:OnAfterExecuting(F,E,T) if prev then prev(self,F,E,T) end; state.casExecuting=true; msg("CAS",string.format("Jalalabad AH-64D CAS executing for Honaker at %d ft mission altitude; MOOSE EngageDetected active",CAS_COMBAT_ALTITUDE_FT),10) end
          else fail("CAS dispatch failed: "..tostring(why)) end
        end
        return demand,created,reason
      end},anchorCoordinate=state.guardCoord,installationId=INSTALLATION_ID,zoneName="OMW_SECURITY_BLUE_GROUND_COP_HONAKER_STAGE3_E2E",priority=90,radiusM=SECURITY_RADIUS_M,
        ...
        onThreatStarted=function(_,opsZone,demand,created,reason,incident)
          ...
          state.threatStarted=true; state.incident=incident
          msg("THREAT","COP Honaker under attack - MOOSE OPSZONE Attacked confirmed",12)
          dispatchQrf(opsZone)
          msg("FIRE SUPPORT",string.format("Waiting %d s for the existing OPSZONE target picture before coordinate fire",FIRE_TARGET_ACQUIRE_DELAY_SEC),10)
          SCHEDULER:New(nil,function() if not state.failed then dispatchFire(opsZone,incident) end end,{},FIRE_TARGET_ACQUIRE_DELAY_SEC)
        end })
      state.threat:Start(); msg("READY","Honaker full-response acceptance armed: QRF + armed CAS + Wright coordinate ARTY + rearm + CH-47 Air-AMMO",15)
    end,{},5)
  end
  state.brigade:Start()
end
...
local function start()
  ...
  need(GROUP:FindByName(INF_TEMPLATE),INF_TEMPLATE); if not context() or not prepareAirwing() then return end
  installAirObserver(); if not preconditionWright() then return end; if not setupFireSupport() then return end
  state.casAdapter=CasAdapter.New({ missionDemand=MissionDemand,registry=registry,airwing=state.airwing,assigneeId="AIRWING:AW_US_JBAD_TF_SHOOTER_6_6_CAV",
    casAltitudeFt=CAS_COMBAT_ALTITUDE_FT,casSpeedKts=CAS_SPEED_KTS,engageDetectedRangeNm=CAS_ENGAGE_RANGE_NM,
    engageDetectedTargetTypes={"Ground Units"},requireExecutionEvidence=true })
  setupDefenceAndThreat()
end

local function finish()
  if state.failed or state.passed then return end
  if not (state.threatStarted and state.qrfEngaged and state.casExecuting and state.casCorridor and state.casFired and state.fireStarted and state.fireComplete and state.rearmComplete and state.supportReturned and state.resupply and state.inTransit and state.delivered and state.airCorridor and state.homeLanded and state.assetReturned) then return end
  ...
  local fd=state.fireDemand and registry:Get(state.fireDemand.id) or nil; local cd=state.casDemand and registry:Get(state.casDemand.id) or nil; local rd=registry:Get(RESUPPLY_DEMAND_ID)
  ...
  if state.fireTargetCompleteCount ~= state.fireTargetCount or state.fireTargetCount < 1 then fail("not all Wright coordinate fire targets completed") return end
  ...
  state.passed=true
  msg("PASS",string.format("Honaker full response complete: QRF + armed CAS via WEST 2500 ft AGL Column + %d Wright coordinate fire missions + M1083 rearm + CampaignState threshold + CH-47 Air-AMMO + Wright 30/30",state.fireTargetCount),30)
