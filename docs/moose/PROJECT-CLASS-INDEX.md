...
| `COHORT` | `VALIDATED_FOR_DOCUMENTED_SCOPE` | post-start `AddAsset()` setzt `squadname`, `legion`, `cohort` und `assets`; Foundation-Läufe bestätigen die registrierte SQUADRON-/Warehouse-Kette; für Acceptance-4 ist `CanMission()` inklusive Missionstyp-/Range-Prüfung source-reviewed, ohne neuen DCS-Nachweis |
| `FLIGHTGROUP` | `VALIDATED_FOR_DOCUMENTED_SCOPE` | AIRWING-`FlightOnMission`-Pfad, Cold-Takeoff-Prüfung und `SetOptionPreferVertical()`-Propagation im finalen Shindand-Lauf bestätigt; AAR source-reviewed für `GetFuelMin()`, `SetFuelLowThreshold()`, `SetFuelLowRTB(false)`, FuelLow-Callback, aktuelle Gruppenkoordinate sowie den Receiver-Pfad `IsAirborne() -> Refuel() -> Going4Fuel -> Refueled`; physisches Boom-Refueling bleibt Acceptance-pflichtig |
...
| `AUFTRAG` | `VALIDATED_FOR_DOCUMENTED_SCOPE` | Salerno CAS sowie Shindand `NewCAS()`, `NewLANDATCOORDINATE()` und `AssignSquadrons()` im nativen AIRWING-Pfad bis Missionserfolg bestätigt; physische Außenlandung bei `LANDATCOORDINATE` nicht beobachtet; für AAR sind `NewTANKER()`, `SetRadio()`, `SetTACAN()`, `SetMissionEgressCoord()`, `IsExecuting()`, `Cancel()` sowie Acceptance-4-`SetMissionRange()` source-reviewed; vollständiger OMW-AAR-Lifecycle und F-16-Range-Korrektur bleiben DCS-pflichtig |
...
Für den aktuellen AAR-Planungs- und Acceptance-Stand sind `AUFTRAG:NewTANKER()`, `AUFTRAG:SetRadio()`, `AUFTRAG:SetTACAN()`, `AUFTRAG:SetMissionEgressCoord()`, `AUFTRAG:IsExecuting()`, `AUFTRAG:Cancel()`, `FLIGHTGROUP:GetFuelMin()`, `FLIGHTGROUP:SetFuelLowThreshold()`, `FLIGHTGROUP:SetFuelLowRTB(false)`, der FuelLow-Callback, die Gruppenkoordinate, `COORDINATE:Get2DDistance()`, `OPSGROUP:Despawn(Delay, NoEventRemoveUnit)`, `AIRWING:CheckTANKER()`, `AIRWING:GetTankerForFlight()` und `COMMANDER:AddTankerZone()` im tatsächlich gepinnten `Moose.lua` geprüft. Für Acceptance-3/4 sind zusätzlich `AUFTRAG:NewCAS()`, `AUFTRAG:SetMissionRange()`, `AUFTRAG:AssignSquadrons()`, `AUFTRAG:AddRequiredPayload()`, `AUFTRAG:SetRequiredAssets()`, `AUFTRAG:CountOpsGroups()`, `COHORT:CanMission()`, `AIRWING:AddMission()`, `AIRWING:OnAfterFlightOnMission`, `FLIGHTGROUP:IsAirborne()`, `FLIGHTGROUP:Refuel()` sowie der `Refuel -> Going4Fuel -> Refueled`-FSM-Pfad einschließlich `OnAfterRefueled` source-reviewed. Der Source-Review bestätigt API-Verfügbarkeit und Signaturen, nicht das reale DCS-Verhalten von KC-135-Orbit, Funk, TACAN, Fuel, Offload, Egress, Off-map-Handoff oder AI-Boom-Transfer.

`COHORT:CanMission()` prüft im gepinnten Stand die Target-Distanz gegen `max(COHORT engageRange, Mission engageRange)`. Acceptance-4 nutzt deshalb für den bestehenden Bagram-F-16C ausschließlich am Test-AUFTRAG `SetMissionRange(250)`, statt die produktive SQUADRON-Reichweite zu verändern. Die dokumentierten Bagram-/Clancy-Koordinaten ergeben rund 227 NM Luftlinie; der Range-Override ist bis zum DCS-Lauf nur `SOURCE_REVIEWED / TEST_PENDING`.

`AIRWING:CheckTANKER()` zeigt, dass MOOSE mehrere Tanker an demselben Patrolpunkt technisch vorsehen kann, verwendet intern jedoch nur ein 1.000-ft-Inkrement je belegtem Slot. OMW legt für zwei **unabhängige** Tanker im selben AAR-Gebiet eine strengere Planungsregel fest: SLOW-Tanker unten, FAST-Tanker oben und mindestens 3.000 ft vertikale Tanker-zu-Tanker-Staffelung. `AIRWING:GetTankerForFlight()` unterscheidet bei gleichem Refueling-System nicht automatisch nach SLOW-/FAST-Receiverprofil, sondern wählt nach Distanz; die receiverbezogene Zuordnung bleibt daher separat Acceptance-pflichtig.

`AAR-KC135-RUNTIME-ACCEPTANCE-2` bestätigte im Owner-Lauf vom 14.08.2026 alle fünf Tanker bis `EXECUTING`, den 180-s-Dwell, FuelLow/Cancel/Egress und den 10-NM-Off-map-Handoff; die Racetracks wurden visuell bestätigt. Der Fünf-Tanker-Lauf bleibt eine Testausnahme. Die dabei beobachtete lokale Spawn-Häufung führt zur produktiven Regel von mindestens 60 Sekunden Abstand innerhalb derselben Gate-/Runtime-Domäne; verschiedene Gate-Domänen dürfen gleichzeitig materialisieren.
...
`AAR-KC135-RUNTIME-ACCEPTANCE-4` korrigiert die DCS-Runtime-TACAN-Bänder auf Y, setzt die Materialisierungsrichtung über `HeadingTo()`/`InitHeading()`, verwendet Clancy mit 220 KIAS und ergänzt den test-only 250-NM-`SetMissionRange()`-Override für den bestehenden Bagram-F-16C. Es wird kein neues Mission-Editor-Template erzeugt und keine `.miz` automatisiert verändert.
