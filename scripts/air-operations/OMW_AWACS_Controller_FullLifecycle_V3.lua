-- Operation Mountain Watch - full external E-3 AWACS lifecycle controller, revision 5.
--
...
--   * MOOSE Refuel pauses the current mission; the paused APOC mission is resumed by the normal
--     OPSGROUP/FLIGHTGROUP lifecycle after refuelling.
--   * CampaignState remains the sole strategic aircraft authority.
--
-- Flight-profile baseline derived from AWACS Acceptance 5:
--   * normal visible WIZARD transit: FL350 / 270 KIAS target;
--   * optional fast WIZARD transit: FL350 / 290 KIAS target (not selected automatically here);
--   * APOC racetrack: FL320 / 250 KIAS;
--   * dedicated LISA AAR racetrack/contact: FL250 / 270 KIAS;
--   * WIZARD dedicated-LISA rendezvous route: FL250 / 290 KIAS target before MOOSE Refuel.
--
-- FLIGHTGROUP:AddWaypoint() and SPAWN:InitSpeedKnots() are route/TAS inputs. WIZARD KIAS targets
-- are therefore converted with the same UTILS.IasToTas() path that was exercised in Acceptance 5.
-- AUFTRAG racetrack speeds remain mission IAS values. The pinned FLIGHTGROUP:Refuel() API has no
-- independent rendezvous/contact speed arguments; it uses the flight group's default cruise speed
-- for its receiver route and then hands final join/contact to the DCS refuelling task.
--
-- Fuel policy:
--   * <=65 percent: pre-dispatch dedicated reserve tanker LISA.
--   * LISA ready: WIZARD begins planned AAR immediately.
--   * <=40 percent: fallback AAR trigger if the planned LISA path is not ready.
--   * <=25 percent without an established refuel task: controlled visible off-map contingency.
--   * LISA FuelLow egress is deferred while WIZARD is actively refuelling from LISA.
--   * MOOSE automatic FuelLow/FuelCritical RTB is disabled.
--   * FLIGHTGROUP:SetFuelLowRefuel(false) is intentional because its pinned implementation uses
--     a fixed 50-NM tanker search that cannot express LISA priority plus OMW's wider fallback.

OMW = OMW or {}

local Controller = {}

local TAG = "[OMW][AWACS.FullLifecycleV3]"
...
local TRANSIT_IAS_KT = 270
local FAST_TRANSIT_IAS_KT = 290
...
local AAR_RENDEZVOUS_ALTITUDE_FT = 25000
local AAR_RENDEZVOUS_IAS_KT = 290
...
local LISA_TRACK_SPEED_KIAS = 270
...
local LISA_SPAWN_INITIAL_SPEED_KT = 480
...
local function requireMoose()
  ...
  if type(UTILS.IasToTas) ~= "function" then
    fail("UTILS.IasToTas() is unavailable")
  end
...
local function iasRouteSpeedKnots(iasKt, altitudeFt)
  return UTILS.IasToTas(iasKt, UTILS.FeetToMeters(altitudeFt))
end
...
local function addPersistentOrbit(runtime, serviceState, reason)
  ...
  local mission = AUFTRAG:NewORBIT_RACETRACK(
    runtime.trackCoord, TRACK_ALTITUDE_FT, TRACK_SPEED_KIAS, TRACK_HEADING_DEG, TRACK_LEG_NM)
  ...
  log(string.format("PERSISTENT_ORBIT runtime=%s altitudeFt=%d speedKIAS=%d serviceState=%s",
    runtime.runtimeId, TRACK_ALTITUDE_FT, TRACK_SPEED_KIAS, runtime.serviceState))
  return true
end
...
local function routeDirectEgress(runtime, reason)
  ...
  runtime.flightGroup:SetDefaultSpeed(runtime.transitRouteSpeedKt)
  setSensorState(runtime, false, "EGRESS")
  cancelOrbitForTrueEgress(runtime)
  local waypoint = runtime.flightGroup:AddWaypoint(
    runtime.firCoord, runtime.transitRouteSpeedKt, nil, TRANSIT_ALTITUDE_FT, true)
  runtime.firEgressWaypointUid = waypoint and waypoint.uid or nil
  log(string.format(
    "EGRESS_ORDERED runtime=%s reason=%s target=%s altitudeFt=%d targetKIAS=%d routeSpeedKt=%.1f",
    runtime.runtimeId, tostring(reason), FIR_FIX_NAME, TRANSIT_ALTITUDE_FT, TRANSIT_IAS_KT,
    runtime.transitRouteSpeedKt))
  return true
end
...
local function dispatchLisa(reason)
  ...
  spawner:InitSpeedKnots(LISA_SPAWN_INITIAL_SPEED_KT)
  ...
  local mission = AUFTRAG:NewTANKER(
    coords.rendezvous, LISA_TRACK_ALTITUDE_FT, LISA_TRACK_SPEED_KIAS,
  ...
  function flightGroup:OnAfterPassingWaypoint(From, Event, To, Waypoint)
    ...
    if Waypoint.uid == lisa.lateWaypointUid and not lisa.latePassed then
      ...
      log(string.format(
        "LISA_LATE_APPROACH_PASSED runtime=%s action=ADD_TANKER_MISSION targetAltitudeFt=%d targetKIAS=%d",
        lisa.runtimeId, LISA_TRACK_ALTITUDE_FT, LISA_TRACK_SPEED_KIAS))
    end
  end
  ...
  local firWp = flightGroup:AddWaypoint(
    coords.daverIn, LISA_TRANSIT_SPEED_KT, nil, LISA_INGRESS_ALTITUDE_FT, false)
  local lateWp = flightGroup:AddWaypoint(
    coords.late, LISA_TRANSIT_SPEED_KT, firWp.uid, LISA_INGRESS_ALTITUDE_FT, true)
  ...
  state.lisa = lisa
  log(string.format(
    "LISA_DISPATCHED runtime=%s reason=%s rendezvousLat=%.6f rendezvousLon=%.6f trackAltitudeFt=%d trackSpeedKIAS=%d",
    lisa.runtimeId, tostring(reason), LISA_RENDEZVOUS.lat, LISA_RENDEZVOUS.lon,
    LISA_TRACK_ALTITUDE_FT, LISA_TRACK_SPEED_KIAS))
...
local function selectTanker(runtime)
  ...
  if tanker then return tanker, "NEAREST_COMPATIBLE_FALLBACK" end
  return nil, "NO_COMPATIBLE_TANKER"
end

local function isDedicatedLisa(tanker)
  local lisa = state.lisa
  return lisa and lisa.group and lisa.group:IsAlive() and tanker
    and tanker:GetName() == lisa.group:GetName()
end
...
local function beginRefuel(runtime, tanker, selectionReason)
  ...
  local dedicatedLisa = isDedicatedLisa(tanker)
  if dedicatedLisa then
    runtime.flightGroup:SetDefaultSpeed(runtime.aarRendezvousRouteSpeedKt)
  else
    runtime.flightGroup:SetDefaultSpeed(runtime.transitRouteSpeedKt)
  end
  ...
  runtime.aarDedicatedLisa = dedicatedLisa

  log(string.format(
    "AAR_TANKER_SELECTED runtime=%s tanker=%s reason=%s fuelPct=%.2f dedicatedLisa=%s rendezvousTargetAltFt=%s rendezvousTargetKIAS=%s routeSpeedKt=%.1f action=MOOSE_REFUEL",
    runtime.runtimeId, runtime.designatedTankerGroupName, tostring(selectionReason),
    runtime.flightGroup:GetFuelMin(), tostring(dedicatedLisa),
    dedicatedLisa and tostring(AAR_RENDEZVOUS_ALTITUDE_FT) or "TANKER_GEOMETRY",
    dedicatedLisa and tostring(AAR_RENDEZVOUS_IAS_KT) or "TANKER_GEOMETRY",
    dedicatedLisa and runtime.aarRendezvousRouteSpeedKt or runtime.transitRouteSpeedKt))

  -- FLIGHTGROUP:Refuel() remains the sole receiver-task implementation. It pauses the
  -- persistent mission and uses the flight group's default cruise speed for its route.
...
local function tryAcquireTanker(runtime)
  ...
  if tanker then return beginRefuel(runtime, tanker, reason) end
...
local function materialize(role, reason)
  ...
  local transitRouteSpeedKt = iasRouteSpeedKnots(TRANSIT_IAS_KT, TRANSIT_ALTITUDE_FT)
  local fastTransitRouteSpeedKt = iasRouteSpeedKnots(FAST_TRANSIT_IAS_KT, TRANSIT_ALTITUDE_FT)
  local aarRendezvousRouteSpeedKt = iasRouteSpeedKnots(AAR_RENDEZVOUS_IAS_KT, AAR_RENDEZVOUS_ALTITUDE_FT)
  ...
  state.awacsSpawner:InitSpeedKnots(transitRouteSpeedKt)
  ...
  flightGroup:SetDefaultSpeed(transitRouteSpeedKt)
  flightGroup:SetFuelLowRTB(false)
  ...
  local runtime = {
    ...
    transitRouteSpeedKt = transitRouteSpeedKt,
    fastTransitRouteSpeedKt = fastTransitRouteSpeedKt,
    aarRendezvousRouteSpeedKt = aarRendezvousRouteSpeedKt,
    firWaypointUid = nil,
    lateWaypointUid = nil,
    firPassed = false,
    latePassed = false,
  ...
  function flightGroup:OnAfterPassingWaypoint(From, Event, To, Waypoint)
    ...
    if Waypoint.uid == runtime.firWaypointUid and not runtime.firPassed then
      ...
      log(string.format("FIR_INGRESS_PASSED runtime=%s fix=%s altitudeFt=%d targetKIAS=%d",
        runtime.runtimeId, FIR_FIX_NAME, TRANSIT_ALTITUDE_FT, TRANSIT_IAS_KT))
      return
    end
  ...
  function flightGroup:OnAfterRefueled(From, Event, To)
    ...
    runtime.flightGroup:SetDefaultSpeed(runtime.transitRouteSpeedKt)
    log(string.format(
      "AAR_REFUELED runtime=%s tanker=%s dedicatedLisa=%s fuelPct=%.2f restoredTransitTargetKIAS=%d restoredRouteSpeedKt=%.1f",
      runtime.runtimeId, tostring(runtime.designatedTankerGroupName), tostring(runtime.aarDedicatedLisa),
      runtime.flightGroup:GetFuelMin(), TRANSIT_IAS_KT, runtime.transitRouteSpeedKt))
  ...
  local firWp = flightGroup:AddWaypoint(
    coords.rosie, transitRouteSpeedKt, nil, TRANSIT_ALTITUDE_FT, false)
  local lateWp = flightGroup:AddWaypoint(
    coords.lateApproach, transitRouteSpeedKt, firWp.uid, TRANSIT_ALTITUDE_FT, true)
  ...
  if type(fuelPct) == "number" and math.abs(fuelPct - EXPECTED_SPAWN_FUEL_PCT) > SPAWN_FUEL_TOLERANCE_PCT then
    log(string.format(
      "SPAWN_FUEL_WARNING runtime=%s observedPct=%.2f expectedPct=%d tolerancePct=%d action=CHECK_ME_TEMPLATE_FUEL",
    ...
    log(string.format("SPAWN_FUEL_ACCEPTED runtime=%s observedPct=%s expectedPct=%d",
      runtime.runtimeId, type(fuelPct) == "number" and string.format("%.2f", fuelPct) or "NA",
      EXPECTED_SPAWN_FUEL_PCT))
  end

  log(string.format(
    "MATERIALIZED runtime=%s source=%s altitudeFt=%d targetKIAS=%d routeSpeedKt=%.1f fuelLowPct=%d fuelCriticalPct=%d lisaPredispatchPct=%d",
    runtime.runtimeId, SOURCE_DOMAIN, TRANSIT_ALTITUDE_FT, TRANSIT_IAS_KT, transitRouteSpeedKt,
...
local function monitorLisa()
  local lisa = state.lisa
  if not lisa or lisa.lossHandled or lisa.handoffComplete
      or not lisa.flightGroup or not lisa.flightGroup:IsAlive() then return end
  ...
  if lisa.missionAdded and not lisa.onStation and not lisa.egressOrdered then
    ...
    if distanceNm and distanceNm <= LISA_TRACK_ENTRY_RADIUS_NM and altitudeReady then
      ...
      lisa.readyAt = now()
      log(string.format(
        "LISA_READY runtime=%s distanceNm=%.1f altitudeFt=%.0f targetAltitudeFt=%d trackSpeedKIAS=%d",
        lisa.runtimeId, distanceNm, altitudeFt, LISA_TRACK_ALTITUDE_FT, LISA_TRACK_SPEED_KIAS))
      ...
      if runtime and not runtime.egressOrdered and not runtime.lossHandled and not runtime.handoffComplete
        ...
        if requireRefuel(runtime, "LISA_READY_ON_RENDEZVOUS") then tryAcquireTanker(runtime) end
      end
    end
  end
  ...
  if lisa.egressOrdered then
    if not lisa.firEgressPassed then
      ...
      if distanceNm and distanceNm <= FIR_RADIUS_NM then
        ...
        lisa.flightGroup:AddWaypoint(
          lisa.handoffCoord, LISA_TRANSIT_SPEED_KT, nil, LISA_EGRESS_ALTITUDE_FT)
...
local function monitorAwacs(runtime)
  ...
  if runtime.aarPhase == "SEEKING_TANKER" then tryAcquireTanker(runtime) end
  ...
  if localSec >= SERVICE_END_SEC and not runtime.egressOrdered then
    if runtime.aarPhase == "REFUELING" or runtime.aarPhase == "SEEKING_TANKER" then
      if not runtime.closePending then
        ...
        log(string.format("SERVICE_CLOSE_PENDING runtime=%s aarPhase=%s",
          runtime.runtimeId, tostring(runtime.aarPhase)))
      end
    else
  ...
  if runtime.egressOrdered then
    if not runtime.firEgressPassed then
      ...
      if distanceNm and distanceNm <= FIR_RADIUS_NM then
        ...
        runtime.flightGroup:AddWaypoint(
          runtime.externalHandoffCoord, runtime.transitRouteSpeedKt, nil, TRANSIT_ALTITUDE_FT)
...
function Controller.Start()
  ...
  state.started = true
  log("STARTED mode=FULL_FUEL_DRIVEN_AAR_V5")
  return runtime
end
...
function Controller.RequestRefuel(rendezvousCoordinate, designatedTankerGroupName)
  ...
  if not runtime or not runtime.flightGroup or not runtime.flightGroup:IsAlive() then
    return false, "AWACS_NOT_AVAILABLE"
  end
  if runtime.egressOrdered or runtime.lossHandled or runtime.handoffComplete then
    return false, "AWACS_NOT_REFUELABLE_STATE"
  end
  ...
  if not tanker then tanker = runtime.flightGroup:FindNearestTanker(TANKER_SEARCH_RADIUS_NM) end
  ...
  if not tanker and rendezvousCoordinate then
    ...
    runtime.aarSelectionReason = "MANUAL_COORDINATE"
    runtime.aarDedicatedLisa = false
...
function Controller.GetConfig()
  ...
  return {
    ...
    spawnInitialSpeedKt = transitRouteSpeedKt,
    inboundCruiseAltitudeFt = TRANSIT_ALTITUDE_FT,
    outboundCruiseAltitudeFt = TRANSIT_ALTITUDE_FT,
    transitTargetIASKt = TRANSIT_IAS_KT,
    transitRouteSpeedKt = transitRouteSpeedKt,
    fastTransitTargetIASKt = FAST_TRANSIT_IAS_KT,
    fastTransitRouteSpeedKt = fastTransitRouteSpeedKt,
    trackAltitudeFt = TRACK_ALTITUDE_FT,
    trackSpeedKIAS = TRACK_SPEED_KIAS,
    ...
    aarRendezvousAltitudeFt = AAR_RENDEZVOUS_ALTITUDE_FT,
    aarRendezvousTargetIASKt = AAR_RENDEZVOUS_IAS_KT,
    aarRendezvousRouteSpeedKt = aarRendezvousRouteSpeedKt,
    lisaRendezvous = { lat = LISA_RENDEZVOUS.lat, lon = LISA_RENDEZVOUS.lon },
    lisaRendezvousAltitudeFt = LISA_TRACK_ALTITUDE_FT,
    lisaRendezvousSpeedKIAS = LISA_TRACK_SPEED_KIAS,
    ...
    refuelUsesMooseTask = true,
    dedicatedLisaRendezvousUsesDefaultSpeed = true,
    finalContactSpeedDcsControlled = true,
