...
$src=Join-Path $repoRoot 'mission\tests\fire-support-strategic-resupply-production-base-runtime\src\08-c2-routed-cas-release-acceptance.lua'
$out=Join-Path $repoRoot 'mission\tests\fire-support-strategic-resupply-production-base-runtime\dist\OMW_FireSupStratResupply_Production_Base_Acceptance_8.lua'
...
foreach($marker in @(
  ...
  'source=AUFTRAG_IsExecuting',
  'Execution watchdog elapsed; lifecycle monitoring continues',
  ...
  if(-not $all.Contains($marker)){throw "Acceptance 8 marker missing: $marker"}
}
...
foreach($forbidden in @(
  ...
  if($test.Contains($forbidden)){throw "Acceptance 8 forbidden selection/runtime shortcut found: $forbidden"}
}

if($test -match 'RouteGroundTo\s*\(|RouteTo\s*\(|SetTask\s*\(|PushTask\s*\('){
  throw 'Acceptance 8 must not rewrite the RED fixture or CAS route with native/custom controller tasks.'
}
if($test -match 'ReportInstallationEvidence\s*\('){
  throw 'Acceptance 8 must not inject installation evidence directly.'
}
if($test -match 'Mission:Cancel\s*\('){
  throw 'Acceptance 8 must close CAS through the Base/CommanderBridge handle, not by direct mission mutation.'
}
if(-not $test.Contains('SELECTED_PROVIDER_HAS_NO_OWNER_ROUTE_PROFILE')){
  throw 'Acceptance 8 requires fail-closed provider/profile validation before physical dispatch.'
}
if(-not $test.Contains('CAS_FUEL_LOW_BEFORE_PHYSICAL_RETURN')){
  throw 'Acceptance 8 requires explicit FuelLow regression detection.'
}
...
Write-Host 'ExecutionWatchdogSec: 3600 diagnostic only; lifecycle monitoring continues'
