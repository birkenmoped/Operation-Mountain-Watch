...
assert(config.configurationVersion == "TM02W2F-red-direct-offroad-watchdog-9",
...
assert(config.routing.physicalMode
    == "DIRECT_OFFROAD_WITH_EXPOSURE_GUARDED_RECOVERY",
  "unexpected physical mode")
assert(config.routing.maximumPhysicalWaypointsPerLeg == 4,
  "recovery routes must remain bounded to four waypoints")
...
assert(config.navigation.combatCooldownSeconds == 180,
  "combat recovery suppression must be 180 seconds")
assert(config.navigation.hitCooldownSeconds == 300,
  "hit recovery suppression must be 300 seconds")
assert(config.watchdog.enabled == true, "watchdog must be enabled")
assert(config.watchdog.routeRefreshAttempts == 2,
  "two route refreshes are required before relocation")
assert(config.watchdog.localDetourAttempts == 4,
  "four local detours are required before relocation")
assert(config.watchdog.proxyMaxRelocationsPerEpisode == 10,
  "proxy episode relocation budget mismatch")
assert(config.watchdog.proxyRelocationAdvanceMeters == 75,
  "proxy relocation distance mismatch")
assert(config.watchdog.proxyMaxRelocationsPerLeg == 24,
  "proxy leg relocation limit mismatch")
assert(config.watchdog.fullGroupMaxRelocationsPerEpisode == 6,
  "full-group episode relocation budget mismatch")
assert(config.watchdog.fullGroupRelocationAdvanceMeters == 40,
  "full-group relocation distance mismatch")
assert(config.watchdog.fullGroupMaxRelocationsPerLeg == 12,
  "full-group leg relocation limit mismatch")
assert(config.watchdog.recoveryCreditProgressMeters == 150,
  "recovery credit threshold mismatch")
assert(config.watchdog.roadRecoveryResetProgressMeters == 300,
  "road recovery reset threshold mismatch")
assert(config.watchdog.recoveryExhaustedRetrySeconds == 60,
  "exhausted recovery retry mismatch")
assert(config.watchdog.exposureClearSeconds == 120,
  "exposure clear gate mismatch")
...
print("TM02W2F static PASS: tasks=20 astar=20 fallback=1 watchdog9 proxy/full exposure-guarded")
