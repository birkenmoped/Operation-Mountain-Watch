...
local function prepareSpawn(k,s,tpl)
  ...
  local a,b=c[1]:GetVec2(),c[2]:GetVec2();local l=dist(a,b);if l<=0 then return false,"FIRST_LEG_TOO_SHORT" end
  local spacing=math.min(SPACING,(l-1)/math.max(1,n-1));if spacing<MINSPACING then return false,"FIRST_LEG_TOO_SHORT_FOR_COMPACT_SPAWN" end
  local h=heading(a,b);local lead=(n-1)*spacing;s.positions={};s.spacing=spacing;s.heading=h
  for i=1,n do local p=point(a,b,lead-(i-1)*spacing);if not p then return false,"SPAWN_POINT_BUILD_FAILED_"..i end;s.positions[i]={x=p.x,y=p.y,heading=h} end
  s.lead=COORDINATE:NewFromVec2(s.positions[1])
  log(string.format("COMPACT_SPAWN_PREPARED siteId=%s units=%d spacingM=%.2f headingDeg=%.1f anchor=PATHLINE_FIRST_SEGMENT",s.site.siteId,n,spacing,h));return true
end
...
local function installSpawn(k,s)
  ...
  s.bde._SpawnAssetGroundNaval=function(self,alias,asset,request,spawnzone,late)
    ...
    log(string.format("COMPACT_ALIGNED_WAREHOUSE_SPAWN siteId=%s units=%d spacingM=%.2f headingDeg=%.1f anchor=PATHLINE_FIRST_SEGMENT",s.site.siteId,#s.positions,s.spacing,s.heading))
...
local function setup(k,tpl)
  ...
  s.bde=BRIGADE:New(site.warehouseName,"BDE_G5_GUARD_A2_"..k)
...
local function start()
  ...
  SCHEDULER:New(nil,telemetry,{},STEP,STEP);SCHEDULER:New(nil,finish,{},OBS);msg("READY","compact/aligned six-site Guard acceptance started; PATHLINE materialization; 2 m interval; 300 s observation",15)
end
