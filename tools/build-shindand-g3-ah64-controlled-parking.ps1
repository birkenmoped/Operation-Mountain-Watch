[CmdletBinding()]
param()

$ErrorActionPreference = 'Stop'
Set-StrictMode -Version Latest

$repoRoot = Split-Path -Parent $PSScriptRoot
$sourceFile = Join-Path $repoRoot 'mission\tests\shindand-air-operations\src\03-shindand-g3-ah64-controlled-parking.lua'
$distDir = Join-Path $repoRoot 'mission\tests\shindand-air-operations\dist'
$outputFile = Join-Path $distDir 'OMW_AirOps_Shindand_G3_AH64_ControlledParking.lua'
$builderVersion = 'SHND-G3-AH64-CONTROLLED-PARKING-1'

if (-not (Test-Path -LiteralPath $sourceFile -PathType Leaf)) {
    throw "Shindand G3 source not found: $sourceFile"
}

$source = Get-Content -LiteralPath $sourceFile -Raw -Encoding UTF8

$requiredMarkers = @(
    '[OMW][AirOps.SHND.G3.AH64Parking]',
    'SQ_US_SHND_AH64D_ATTACK',
    'TPL_AIR_US_SHND_AH64D_CAS_2SHIP',
    'EXPECTED_PARKING_IDS = { 21, 3, 34, 15 }',
    'airwing:SetParkingIDs(EXPECTED_PARKING_IDS)',
    'WAREHOUSE.Descriptor.GROUPNAME',
    'airwing:AddRequest(',
    'airwing:OnAfterSelfRequest',
    'GetClosestParkingSpot',
    'PASS_CONTROLLED_PARKING',
    'nativeWarehouseSelfRequest=true',
    'auftrag=false',
    'commander=false'
)
foreach ($marker in $requiredMarkers) {
    if (-not $source.Contains($marker)) {
        throw "G3 source is missing required marker: $marker"
    }
}

$forbiddenPatterns = @(
    'COMMANDER\s*:\s*New',
    'AUFTRAG\s*:\s*New',
    'AIRWING\s*:\s*AddMission',
    'OPSTRANSPORT\s*:\s*New',
    'missionCommands',
    'MENU_COALITION',
    'MENU_MISSION',
    'SPAWN\s*:',
    'coalition\s*\.\s*addGroup',
    'mist\s*\.',
    '(?<![A-Za-z0-9_])CampaignState\s*(?:[\.:=\[]|\()',
    '_SpawnAssetAircraft\s*=|_FindParkingForAssets\s*=',
    'trigger\.action\.setUserFlag',
    'trigger\.action\.markTo'
)

$scanSource = [regex]::Replace($source, '(?m)^\s*--.*(?:\r?\n|$)', '')
foreach ($pattern in $forbiddenPatterns) {
    if ($scanSource -match $pattern) {
        throw "G3 regression: forbidden pattern found: $pattern"
    }
}

New-Item -ItemType Directory -Path $distDir -Force | Out-Null
if (Test-Path -LiteralPath $outputFile -PathType Leaf) {
    Remove-Item -LiteralPath $outputFile -Force
}

$commit = (& git -C $repoRoot rev-parse HEAD).Trim()
$header = "-- AUTO-GENERATED FILE. DO NOT EDIT DIRECTLY.`n-- Builder: tools/build-shindand-g3-ah64-controlled-parking.ps1`n-- BuilderVersion: $builderVersion`n-- GitCommit: $commit`n-- MOOSE-Pin: 73d3ed119cd9e7e3f2cfcabbaa34513d30529b54`n-- Scope: isolated Shindand AH-64 native AIRWING/WAREHOUSE controlled parking test.`n-- GeneratedUtc: $([DateTime]::UtcNow.ToString('o'))`n`n"
$content = $header + $source
$scanContent = [regex]::Replace($content, '(?m)^\s*--.*(?:\r?\n|$)', '')
foreach ($pattern in $forbiddenPatterns) {
    if ($scanContent -match $pattern) {
        throw "Generated G3 bundle contains forbidden pattern: $pattern"
    }
}

[System.IO.File]::WriteAllText($outputFile, $content, [System.Text.UTF8Encoding]::new($false))

$hash = (Get-FileHash -LiteralPath $outputFile -Algorithm SHA256).Hash.ToLowerInvariant()
Write-Host "Built: $outputFile"
Write-Host "BuilderVersion: $builderVersion"
Write-Host "Scope: SHINDAND_G3_AH64_CONTROLLED_PARKING_ONLY"
Write-Host "RequestPath: AIRWING_WAREHOUSE_SELF_REQUEST"
Write-Host "RequiredAssetGroups: 1"
Write-Host "ExpectedAircraftPerAssetGroup: 2"
Write-Host "AH64ParkingTerminalIDs: 21,3,34,15"
Write-Host "AirwingParkingRestriction: REQUIRED"
Write-Host "Commander: ABSENT"
Write-Host "AUFTRAG: ABSENT"
Write-Host "OPSTRANSPORT: ABSENT"
Write-Host "DirectSpawn: ABSENT"
Write-Host "MOOSEOverride: ABSENT"
Write-Host "CampaignStateMutation: ABSENT"
Write-Host "RequiredGuardPatternsChecked: $($requiredMarkers.Count)"
Write-Host "ForbiddenGuardPatternsChecked: $($forbiddenPatterns.Count)"
Write-Host "SHA256: $hash"
Write-Host "GitCommit: $commit"
