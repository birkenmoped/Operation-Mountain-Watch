...
local FORM,INTERVAL,SPACING,MINSPACING="Off Road",2,2,0.75
...
local function offset(origin,h,d) local r=math.rad(h);return{x=origin.x+math.cos(r)*d,y=origin.y+math.sin(r)*d} end
...
local function prepareSpawn(k,s,tpl)
  ...
  local a,b=c[1]:GetVec2(),c[2]:GetVec2();if dist(a,b)<=0 then return false,"FIRST_LEG_TOO_SHORT" end
  local center=s.access:GetVec2();if not center or s.access:IsVec2InZone(center)~=true then return false,"ACCESS_CENTER_INVALID" end
  local h=heading(a,b);local spacing=SPACING;local positions=nil
  while spacing>=MINSPACING do
    local candidate={};local half=(n-1)*spacing/2;local inside=true
    for i=1,n do
      local p=offset(center,h,half-(i-1)*spacing)
      if s.access:IsVec2InZone(p)~=true then inside=false;break end
      candidate[i]={x=p.x,y=p.y,heading=h}
    end
    if inside then positions=candidate;break end
    spacing=spacing-0.25
  end
  if not positions then return false,"COMPACT_LINE_DOES_NOT_FIT_ACCESS" end
  s.positions=positions;s.spacing=spacing;s.heading=h;s.lead=COORDINATE:NewFromVec2(center)
  log(string.format("COMPACT_SPAWN_PREPARED siteId=%s units=%d spacingM=%.2f headingDeg=%.1f anchor=ACCESS_CENTER pathlineFirstLegDistanceM=%.1f",s.site.siteId,n,spacing,h,dist(center,a)))
  return true
end
...
local function installSpawn(k,s)
  ...
  s.bde._SpawnAssetGroundNaval=function(self,alias,asset,request,spawnzone,late)
    ...
    log(string.format("COMPACT_ALIGNED_WAREHOUSE_SPAWN siteId=%s units=%d spacingM=%.2f headingDeg=%.1f anchor=ACCESS_CENTER",s.site.siteId,#s.positions,s.spacing,s.heading))
...
local function route(g,s)
  local c=s.path:GetCoordinates();if type(c)~="table" or #c<2 or not s.start then return nil end
  local r={s.start:WaypointGround(SPEED,FORM)}
  for i=1,#c do r[#r+1]=c[i]:WaypointGround(SPEED,FORM) end
...
local function setup(k,tpl)
  ...
  s.bde.OnAfterArmyOnMission=function(self,From,Event,To,ag,m)
    ...
    s.group:Route(r,2);s.routeStarted=true;msg("GUARD",k.." compact/aligned from ACCESS center onto "..site.guardRoute.pathlineName,8)
  end
...
local function start()
  ...
  SCHEDULER:New(nil,telemetry,{},STEP,STEP);SCHEDULER:New(nil,finish,{},OBS);msg("READY","compact/aligned six-site Guard acceptance started; ACCESS-center materialization; 2 m interval; 300 s observation",15)
end
