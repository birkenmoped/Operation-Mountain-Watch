...
local function validateMissionGroup(name, expectedType, expectedSize)
  ...
  if not template then log("MISSING GROUP_TEMPLATE " .. name) return false end
...
local function validateStatic(name, expectedType)
  ...
  if not static then log("MISSING STATIC " .. name) return false end
...
local function validateOptionalGroups(groups, expectedCount)
  ...
  for _, name in ipairs(groups or {}) do
    local template = getMissionTemplate(name)
    if template then present[#present + 1] = { Name = name, Template = template } end
  end
  if #present == 0 then
...
local function validateRampModel(cfg)
  ...
  if parking.ComparableHelicopterPositions ~= 36 or
     ...
     parking.TemplateOperationalParkingPositions ~= 0 or
     parking.TemplatesUseOperationalParking ~= false then
    log("ERROR: Parking model must keep all seven technical templates off operational parking while retaining six core Client positions.")
    ok = false
  end

  local pools = parking.SquadronPools or {}
  if not pools.OH58D or #(pools.OH58D.Entries or {}) ~= 5 or
     not pools.AH64D or #(pools.AH64D.Entries or {}) ~= 3 or
     not pools.UH60 or #(pools.UH60.Entries or {}) ~= 3 or
     not pools.CH47 or #(pools.CH47.Entries or {}) ~= 8 then
    log("ERROR: Exclusive SQUADRON parking pools do not match OH58D=5/AH64D=3/UH60=3/CH47=8.")
    ok = false
  end
  ...
  if cfg.ParkingPoolsOK ~= true then
    log("ERROR: Exclusive SQUADRON parking-pool validation did not pass.")
    ok = false
  end
  if ok then
    log("OK RAMP_MODEL inventoryVirtual=true clients=6 optionalClients=2 templateAircraft=7 templatesOnOperationalParking=0 exclusivePools=OH58D:5/AH64D:3/UH60:3/CH47:8 comparablePositions=36 staticCaps=7/4/4/5.")
  end
  return ok
end
...
local function validateMedevacModel(cfg)
  ...
  local ok = true
  if medevac.PackageSize ~= 2 or medevac.LeadAircraft ~= 1 or medevac.CoverAircraft ~= 1 or medevac.AllowSingleShip ~= false then
    log("ERROR: MEDEVAC package policy must be 1 lead + 1 cover with no single-ship fallback.")
    ok = false
  end
  if medevac.DCSGroupModel ~= "TWO_INDEPENDENT_SINGLE_SHIP_GROUPS" or medevac.CoordinationModel ~= "ONE_LOGICAL_MEDEVAC_PACKAGE" then
    log("ERROR: MEDEVAC must use two independently taskable single-ship DCS groups coordinated as one logical package.")
    ok = false
  end
  if ok then log("OK MEDEVAC_MODEL package=2 DCSgroups=1lead+1cover independentTasking=true logicalPackage=true.") end
  return ok
end

local function main()
  local cfg = OMW and OMW.AirOps and OMW.AirOps.Jalalabad
  if not cfg then log("ERROR: Jalalabad configuration is unavailable.") return end

  local ok = true
  if cfg.ParkingPoolsOK ~= true then
    log("ERROR: Exclusive SQUADRON parking pools are not validated.")
    ok = false
  end
  ...
  if not ch47Type then log("ERROR: Canonical CH-47 DCS type was not detected from the heavy-lift template.") ok = false end
  if not cfg.Airwing then log("ERROR: AIRWING is not constructed.") ok = false end

  for key, label in pairs({ OH58D = "OH-58D", AH64D = "AH-64D", UH60 = "UH-60", CH47 = "CH-47" }) do
    if not cfg.Squadrons or not cfg.Squadrons[key] then log("ERROR: " .. label .. " SQUADRON is unavailable.") ok = false end
  end

  if not cfg.Payloads or not cfg.Payloads.OH58DRecon then log("ERROR: OH-58D RECON payload is unavailable.") ok = false end
  if not cfg.Payloads or not cfg.Payloads.AH64DCAS then log("ERROR: AH-64D CAS payload is unavailable.") ok = false end
  if not cfg.Payloads or not cfg.Payloads.UH60MedevacLead or not cfg.Payloads.UH60MedevacCover then log("ERROR: UH-60 MEDEVAC lead/cover payloads are unavailable.") ok = false end
  if not cfg.Payloads or not cfg.Payloads.CH47HeavyLift then log("ERROR: CH-47 heavy-lift payload is unavailable.") ok = false end

  for _, name in ipairs(cfg.PlayerGroups.Required.OH58D or {}) do if not validateMissionGroup(name, "OH58D", 1) then ok = false end end
  for _, name in ipairs(cfg.PlayerGroups.Required.AH64D or {}) do if not validateMissionGroup(name, "AH-64D_BLK_II", 1) then ok = false end end
  if ch47Type then for _, name in ipairs(cfg.PlayerGroups.Required.CH47 or {}) do if not validateMissionGroup(name, ch47Type, 1) then ok = false end end end
  ...
  for _, name in ipairs(cfg.Statics.OH58D or {}) do if not validateStatic(name, "OH58D") then ok = false end end
  for _, name in ipairs(cfg.Statics.AH64D or {}) do if not validateStatic(name, "AH-64D_BLK_II") then ok = false end end
  for _, name in ipairs(cfg.Statics.UH60 or {}) do if not validateStatic(name, "UH-60A") then ok = false end end
  if ch47Type then for _, name in ipairs(cfg.Statics.CH47 or {}) do if not validateStatic(name, ch47Type) then ok = false end end end

  for _, name in ipairs(cfg.Zones or {}) do
    if ZONE:FindByName(name) then log("OK ZONE " .. name) else log("MISSING ZONE " .. name) ok = false end
  end

  local anchor = STATIC:FindByName(cfg.WarehouseName, false) or UNIT:FindByName(cfg.WarehouseName)
  if anchor then log("OK WAREHOUSE_ANCHOR " .. cfg.WarehouseName) else log("MISSING WAREHOUSE_ANCHOR " .. cfg.WarehouseName) ok = false end
  if not validateOptionalGroups(cfg.PlayerGroups.Optional.UH60L, 2) then ok = false end
  ...
  if cfg.Limits.PlayerPerType ~= 2 or cfg.Limits.AIPerType ~= 4 or cfg.Limits.ConcurrentSupportMissions ~= 2 or cfg.Limits.AircraftPerMission ~= 2 or cfg.Limits.ConcurrentSupportAircraft ~= 4 then
    log("ERROR: Air Operations limits do not match player=2 and locked AI policy.")
    ok = false
  end
  if not validateMedevacModel(cfg) then ok = false end
  if not validateRampModel(cfg) then ok = false end
...
if SCHEDULER then
  ...
  timer.scheduleFunction(function() main() return nil end, nil, timer.getTime() + 18)
end
