local TM02W2EMooseNavigationV4 = {}
...
local function segmentDistanceToPoint(first, second, point)
  ...
  return distance2D({ x = a.x + dx * t, z = a.z + dz * t }, p)
end
...
local function pathLength(coordinates)
  local total = 0
...
function TM02W2EMooseNavigationV4.install(config, registryState, plannerState)
  ...
  local function registerPortals()
    for siteId in pairs(registryState.siteById or {}) do
      ...
      if not center then
        addError("SITE_COORDINATE_MISSING", siteId)
      elseif not portal then
        ...
        if blocked then
          ...
          if snapDistanceMeters > (config.navigation.maximumRoadSnapMeters or 1500) then
            addWarning("ROAD_PORTAL_DISTANT", siteId .. " distance=" .. string.format("%.0f", snapDistanceMeters))
          end
  ...
  local function getRoadCandidate(fromCoordinate, toCoordinate)
    ...
    if lengthMeters > (config.navigation.maximumRoadPathMeters or 50000) then
  ...
  local function makeDetourNodes(fromCoordinate, toCoordinate)
    local nodes = {
      { coordinate = fromCoordinate, nodeId = "START" },
      { coordinate = toCoordinate, nodeId = "END" },
    }
    local count = config.navigation.avoidanceRingPointCount or 48
    local clearance = config.navigation.exclusionClearanceMeters or 150
    for exclusionIndex, exclusion in ipairs(navigation.exclusions) do
      local center = vec3(exclusion.coordinate)
      local radius = exclusion.radiusMeters + clearance
      for pointIndex = 0, count - 1 do
        local angle = pointIndex * 2 * math.pi / count
        ...
        if not blocked then
          nodes[#nodes + 1] = {
            ...
            nodeId = "E" .. tostring(exclusionIndex) .. "P" .. tostring(pointIndex + 1),
          }
        end
      end
    end
  ...
  local function customShortestPath(nodes)
    local startIndex = 1
    local endIndex = 2
    local distanceByIndex = { [startIndex] = 0 }
    local previousByIndex = {}
    local visited = {}
    while true do
      local currentIndex
      ...
      for index = 1, #nodes do
        if not visited[index]
          and distanceByIndex[index]
          and distanceByIndex[index] < currentDistance then
          currentIndex = index
          currentDistance = distanceByIndex[index]
        end
      end
      ...
      for neighbourIndex = 1, #nodes do
        if neighbourIndex ~= currentIndex and not visited[neighbourIndex] then
          local blocked = segmentBlocked(nodes[currentIndex].coordinate, nodes[neighbourIndex].coordinate)
          if not blocked then
            local candidate = currentDistance
              + distance2D(nodes[currentIndex].coordinate, nodes[neighbourIndex].coordinate)
            if not distanceByIndex[neighbourIndex] or candidate < distanceByIndex[neighbourIndex] then
              distanceByIndex[neighbourIndex] = candidate
              previousByIndex[neighbourIndex] = currentIndex
            end
          end
        end
      end
    ...
    local indexes = {}
    local currentIndex = endIndex
    while currentIndex do
      table.insert(indexes, 1, currentIndex)
      currentIndex = previousByIndex[currentIndex]
    end
    if indexes[1] ~= startIndex then
      return nil
    end
    local coordinates = {}
    for _, index in ipairs(indexes) do
      coordinates[#coordinates + 1] = nodes[index].coordinate
    end
    return coordinates
  end

  local function mooseAstarPath(nodes)
    local astar = ASTAR:New()
    local mooseNodes = {}
    for index, routeNode in ipairs(nodes) do
      local node = astar:AddNodeFromCoordinate(routeNode.coordinate)
      node.routeNode = routeNode
      mooseNodes[index] = node
    end
    astar:FindStartNode(mooseNodes[1])
    astar:FindEndNode(mooseNodes[2])
    astar:SetValidNeighbourFunction(function(firstNode, secondNode)
      return segmentBlocked(firstNode.routeNode.coordinate, secondNode.routeNode.coordinate) ~= true
    end)
    astar:SetCostFunction(function(firstNode, secondNode)
      return distance2D(firstNode.routeNode.coordinate, secondNode.routeNode.coordinate)
    end)
    local ok, result = pcall(function()
    ...
    for _, node in ipairs(result) do
      coordinates[#coordinates + 1] = node.routeNode.coordinate
    end
    return coordinates
  end

  local function getOffRoadCandidate(fromCoordinate, toCoordinate)
    local directBlocked = segmentBlocked(fromCoordinate, toCoordinate)
    if not directBlocked then
      local direct = { fromCoordinate, toCoordinate }
      return direct, distance2D(fromCoordinate, toCoordinate), "DIRECT_OFFROAD"
    end
    local nodes = makeDetourNodes(fromCoordinate, toCoordinate)
    local coordinates = mooseAstarPath(nodes)
    local method = "MOOSE_ASTAR"
    if not coordinates then
      coordinates = customShortestPath(nodes)
    ...
    local blocked, objectiveId = pathBlocked(coordinates)
    if blocked then
      return nil, nil, "OFFROAD_PATH_BLOCKED:" .. tostring(objectiveId)
    end
    log("INFO", "offroad_detour_selected", {
      method = method,
      pointCount = #coordinates,
      pathDistanceMeters = string.format("%.0f", pathLength(coordinates)),
    })
    return coordinates, pathLength(coordinates), method
  end
  ...
  local function buildHybridPlan(fromCoordinate, toCoordinate)
    ...
    local offRoadCoordinates, offRoadLength, offRoadMethod = getOffRoadCandidate(fromCoordinate, toCoordinate)
  ...
  local function compileMovementEdges()
    for _, link in ipairs(registryState.movementLinks or {}) do
      ...
      if sourcePortal and targetPortal then
        ...
        plan = { safe = false, mode = "UNAVAILABLE", rejectionReason = "ENDPOINT_MISSING" }
      end
      ...
      log(plan.safe and "INFO" or "WARNING", "movement_edge_compiled", {
        linkId = link.linkId,
        ...
        offRoadMethod = plan.offRoadMethod or "none",
      })
      if plan.safe ~= true then
        navigation.unsafeEdgeCount = navigation.unsafeEdgeCount + 1
        addWarning("MOVEMENT_EDGE_UNUSABLE", link.linkId .. " reason=" .. tostring(plan.rejectionReason))
      end
    end
  end
  ...
  local function directedPlan(sourceSiteId, targetSiteId)
    ...
    return {
      ...
      offRoadMethod = plan.offRoadMethod,
  ...
  local function customLogicalPath(sourceSiteId, targetSiteId)
    local siteIds = {}
    for siteId in pairs(navigation.portalsBySiteId) do
      siteIds[#siteIds + 1] = siteId
    end
    table.sort(siteIds)
    ...
    while true do
      ...
      if current == targetSiteId then
        break
      end
      visited[current] = true
      for _, candidate in ipairs(siteIds) do
        if candidate ~= current and not visited[candidate] then
          local plan = navigation.planByPair[pairKey(current, candidate)]
          if plan and plan.safe == true then
            local candidateCost = currentCost + plan.lengthMeters
            if not cost[candidate] or candidateCost < cost[candidate] then
              cost[candidate] = candidateCost
  ...
  local function safeLogicalPath(sourceSiteId, targetSiteId)
    ...
    if not nodesBySiteId[sourceSiteId] or not nodesBySiteId[targetSiteId] then
      return nil, "ENDPOINT_MISSING"
    end
    astar:FindStartNode(nodesBySiteId[sourceSiteId])
    astar:FindEndNode(nodesBySiteId[targetSiteId])
    astar:SetValidNeighbourFunction(function(firstNode, secondNode)
      local plan = navigation.planByPair[pairKey(firstNode.siteId, secondNode.siteId)]
    ...
    if ok and type(result) == "table" and #result >= 2 then
      ...
      return path, "MOOSE_ASTAR"
    end
    return customLogicalPath(sourceSiteId, targetSiteId), "CUSTOM_GRAPH_AFTER_MOOSE_ASTAR"
  end

  function navigation:preparePlannerTasks()
    self.safeTaskCount = 0
    for _, task in ipairs(plannerState.tasks or {}) do
      local path, method = safeLogicalPath(task.sourceSiteId, task.targetSiteId)
      if not path then
    ...
    self.routingReady = #self.errors == 0 and self.safeTaskCount == #(plannerState.tasks or {})
    self.valid = self.routingReady
    log(self.routingReady and "INFO" or "ERROR", "task_routing_validation", {
      ...
      unsafeEdgeCount = self.unsafeEdgeCount,
    })
  ...
  local function sampleCoordinates(coordinates, spacingMeters)
    ...
    for segmentIndex = 2, #coordinates do
      ...
      for step = 1, steps do
        result[#result + 1] = interpolate(first, second, step / steps)
      end
    end
    return result
  end
  ...
  local function wrapProxyGroup(group)
    if not group or group.__OMWTM02W2EV4Wrapped == true then
      return group
    end
    ...
    group.__OMWTM02W2EV4Wrapped = true
  ...
  local function patchProxySpawner()
    if SPAWN.__OMWTM02W2EV4Installed == true then
      return
    end
    SPAWN.__OMWTM02W2EV4Installed = true
    ...
    SPAWN.NewWithAlias = function(spawnClass, templateName, alias)
      ...
      if startsWith(alias, config.proxy.runtimeAliasPrefix) then
        function spawner:SpawnInZone(zone, randomize)
          ...
          local active = TM02W2EMooseNavigationV4.__active
  ...
  local function stopForNavigationFailure(task, reason)
    ...
    log("ERROR", "navigation_blocked", { taskId = task.taskId, reason = reason, recoveryCount = task.navRecoveryCount or 0 })
  end

  local function recoverTask(task, now, reason)
    if taskInCombat(task, now) then
      log("INFO", "watchdog_recovery_suppressed_combat", { taskId = task.taskId, reason = reason })
      return
    end
    ...
    local replacement = SPAWN:NewWithAlias(config.templatesByStrength[task.strength], alias):SpawnFromCoordinate(coordinate)
  ...
  function navigation:attach(executionState)
    ...
    timer.scheduleFunction(function(_, scheduledTime)
      if generation ~= self.watchdogGeneration or executionState.completed == true or executionState.failed == true then
        return nil
      end
  ...
  TM02W2EMooseNavigationV4.__active = navigation
  navigation.valid = #navigation.errors == 0
  if navigation.valid then
    patchProxySpawner()
  end
  local portalCount = 0
  ...
  log("INFO", "navigation_validation", {
    ...
    unsafeEdgeCount = navigation.unsafeEdgeCount,
  })
  return navigation
end

return TM02W2EMooseNavigationV4
