-- Operation Mountain Watch - ARMY Ground Foundation Acceptance 7 harness.
...
local TEST_ID = "ARMY-GROUND-ACCEPTANCE-7-1"
local TAG = "OMW_GND_A7"
...
local sites = {
  { id="FENTY", scenario="NORMAL_RETURN", expectedReturnCount=4, warehouse="WH_BLUE_GND_FENTY", brigadeName="BDE_BLUE_GND_FENTY_A7_NORMAL_RETURN", platoonName="PLT_BLUE_GND_FENTY_A7_MATV", accessZone="ZON_BLUE_GND_FENTY_ACCESS", observationZone="ZON_BLUE_GND_FENTY_PATROL_TEST_01", nodeId="GROUND_NODE_JALALABAD", vehicleResourceId="GROUND:GROUND_NODE_JALALABAD:VEHICLE", personnelResourceId="GROUND:GROUND_NODE_JALALABAD:PERSONNEL", vehicleLossResourceId="GROUND:GROUND_NODE_JALALABAD:VEHICLE_LOST", personnelLossResourceId="GROUND:GROUND_NODE_JALALABAD:PERSONNEL_LOST", runtimeId="ARMY-GROUND-A7-FENTY-1" },
  { id="JOYCE", scenario="PARTIAL_LOSS", expectedReturnCount=3, warehouse="WH_BLUE_GND_JOYCE", brigadeName="BDE_BLUE_GND_JOYCE_A7_PARTIAL_LOSS", platoonName="PLT_BLUE_GND_JOYCE_A7_MATV", accessZone="ZON_BLUE_GND_JOYCE_ACCESS", observationZone="ZON_BLUE_GND_JOYCE_PATROL_TEST_01", nodeId="GROUND_NODE_JOYCE", vehicleResourceId="GROUND:GROUND_NODE_JOYCE:VEHICLE", personnelResourceId="GROUND:GROUND_NODE_JOYCE:PERSONNEL", vehicleLossResourceId="GROUND:GROUND_NODE_JOYCE:VEHICLE_LOST", personnelLossResourceId="GROUND:GROUND_NODE_JOYCE:PERSONNEL_LOST", runtimeId="ARMY-GROUND-A7-JOYCE-1" },
  { id="WRIGHT", scenario="PARTIAL_LOSS_WITH_DAMAGE", expectedReturnCount=3, warehouse="WH_BLUE_GND_WRIGHT", brigadeName="BDE_BLUE_GND_WRIGHT_A7_PARTIAL_LOSS_DAMAGE", platoonName="PLT_BLUE_GND_WRIGHT_A7_MATV", accessZone="ZON_BLUE_GND_WRIGHT_ACCESS", observationZone="ZON_BLUE_GND_WRIGHT_PATROL_TEST_01", nodeId="GROUND_NODE_WRIGHT", vehicleResourceId="GROUND:GROUND_NODE_WRIGHT:VEHICLE", personnelResourceId="GROUND:GROUND_NODE_WRIGHT:PERSONNEL", vehicleLossResourceId="GROUND:GROUND_NODE_WRIGHT:VEHICLE_LOST", personnelLossResourceId="GROUND:GROUND_NODE_WRIGHT:PERSONNEL_LOST", runtimeId="ARMY-GROUND-A7-WRIGHT-1" },
}
...
local function resources(vehicle, personnel)
  return {
    VEHICLE = vehicle, PERSONNEL = personnel,
  }
end

local campaignStore = CampaignState.New({
  schemaVersion = "ARMY-GROUND-ACCEPTANCE-7-1",
  nodes = {
    { nodeId="GROUND_NODE_JALALABAD", airbaseName="Fenty Ground A7", resources={
      ["GROUND:GROUND_NODE_JALALABAD:VEHICLE"]={quantity=48, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JALALABAD:PERSONNEL"]={quantity=480, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JALALABAD:VEHICLE_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JALALABAD:PERSONNEL_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
    } },
    { nodeId="GROUND_NODE_JOYCE", airbaseName="Joyce Ground A7", resources={
      ["GROUND:GROUND_NODE_JOYCE:VEHICLE"]={quantity=20, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JOYCE:PERSONNEL"]={quantity=180, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JOYCE:VEHICLE_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_JOYCE:PERSONNEL_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
    } },
    { nodeId="GROUND_NODE_WRIGHT", airbaseName="Wright Ground A7", resources={
      ["GROUND:GROUND_NODE_WRIGHT:VEHICLE"]={quantity=22, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_WRIGHT:PERSONNEL"]={quantity=120, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_WRIGHT:VEHICLE_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
      ["GROUND:GROUND_NODE_WRIGHT:PERSONNEL_LOST"]={quantity=0, unit=CampaignState.Unit.COUNT},
    } },
  },
})
local groundAdapter = GroundCampaignStateAdapter.New(campaignStore, CampaignState)

local function deploymentSpec(site)
  return { runtimeId=site.runtimeId, nodeId=site.nodeId, vehicleCount=4,
    perVehicleResources={ [site.vehicleResourceId]=1, [site.personnelResourceId]=3 },
    missionDemandId="A7-"..site.id }
end

local function testResourceSnapshot(site)
  return { vehicle=campaignStore:GetResource(site.nodeId, site.vehicleResourceId), personnel=campaignStore:GetResource(site.nodeId, site.personnelResourceId) }
end
local function commitTestDeployment(site)
  local before=testResourceSnapshot(site)
  if before.vehicle.available < 4 or before.personnel.available < 12 then fail(site, "CAMPAIGNSTATE_INITIAL_RESOURCE_INVALID"); return false end
  local commitment, inserted, reason=groundAdapter:OnMaterialized(deploymentSpec(site))
  if not commitment or inserted~=true then fail(site, "CAMPAIGNSTATE_RESERVE_CONSUME_FAILED reason="..tostring(reason)); return false end
  local after=testResourceSnapshot(site)
  if after.vehicle.available~=before.vehicle.available-4 or after.personnel.available~=before.personnel.available-12 then fail(site, "CAMPAIGNSTATE_CONSUME_FAILED"); return false end
  site.campaignCommitted=true
  log("CAMPAIGNSTATE_DEPLOYMENT_COMMITTED site="..site.id.." runtimeId="..site.runtimeId.." vehicle=4 personnel=12")
  return true
end
local function settleTestLoss(site)
  if site.scenario=="NORMAL_RETURN" then return true end
  local result=groundAdapter:OnLost(site.runtimeId, 1, { [site.vehicleResourceId]=site.vehicleLossResourceId, [site.personnelResourceId]=site.personnelLossResourceId })
  if result[site.vehicleResourceId].quantity~=1 or result[site.personnelResourceId].quantity~=3 then fail(site, "CAMPAIGNSTATE_LOSS_SETTLEMENT_FAILED"); return false end
  site.campaignLossSettled=true
  log("CAMPAIGNSTATE_LOSS_RECORDED site="..site.id.." runtimeId="..site.runtimeId.." vehicle=1 personnel=3")
  return true
end
local function settleTestReturn(site)
  ...
  local before=testResourceSnapshot(site)
  local result=groundAdapter:OnReturned(site.runtimeId, site.expectedReturnCount)
  local duplicate=groundAdapter:OnReturned(site.runtimeId, site.expectedReturnCount)
  local after=testResourceSnapshot(site)
  if result[site.vehicleResourceId].inserted~=true or duplicate[site.vehicleResourceId].inserted~=false or result[site.personnelResourceId].inserted~=true or duplicate[site.personnelResourceId].inserted~=false then fail(site, "CAMPAIGNSTATE_EXACTLY_ONCE_FAILED"); return false end
  if after.vehicle.available~=before.vehicle.available+site.expectedReturnCount or after.personnel.available~=before.personnel.available+(site.expectedReturnCount*3) then fail(site, "CAMPAIGNSTATE_RETURN_CREDIT_FAILED"); return false end
  site.campaignSettled=true
  log("CAMPAIGNSTATE_RETURN_CREDIT site="..site.id.." runtimeId="..site.runtimeId.." vehicle="..tostring(site.expectedReturnCount).." personnel="..tostring(site.expectedReturnCount*3))
  log("CAMPAIGNSTATE_EXACTLY_ONCE site="..site.id.." runtimeId="..site.runtimeId)
  return true
end
local function verifyRestartReconciliation()
  local restartStore=CampaignState.New({schemaVersion="A7-RESTART", nodes={{nodeId="GROUND_NODE_JALALABAD", airbaseName="Fenty Restart A7", resources={
    ["GROUND:GROUND_NODE_JALALABAD:VEHICLE"]={quantity=48, unit=CampaignState.Unit.COUNT},
    ["GROUND:GROUND_NODE_JALALABAD:PERSONNEL"]={quantity=480, unit=CampaignState.Unit.COUNT},
  }}}})
  local adapter=GroundCampaignStateAdapter.New(restartStore, CampaignState)
  local spec={runtimeId="ARMY-GROUND-A7-RESTART-1", nodeId="GROUND_NODE_JALALABAD", vehicleCount=4, perVehicleResources={
    ["GROUND:GROUND_NODE_JALALABAD:VEHICLE"]=1, ["GROUND:GROUND_NODE_JALALABAD:PERSONNEL"]=3 }}
  local commitment, inserted=adapter:OnMaterialized(spec)
  if not commitment or inserted~=true then fail(nil, "RESTART_SETUP_FAILED"); return false end
  local restored=CampaignState.Restore(restartStore:ExportSnapshot())
  local restoredAdapter=GroundCampaignStateAdapter.New(restored, CampaignState)
  local reconciliation=restoredAdapter:ReconcileRestore()
  local vehicle=restored:GetResource("GROUND_NODE_JALALABAD", "GROUND:GROUND_NODE_JALALABAD:VEHICLE")
  local personnel=restored:GetResource("GROUND_NODE_JALALABAD", "GROUND:GROUND_NODE_JALALABAD:PERSONNEL")
  if reconciliation.reconciled~=2 or vehicle.available~=48 or personnel.available~=480 then fail(nil, "RESTART_RECONCILIATION_FAILED"); return false end
  log("CAMPAIGNSTATE_RESTART_RECONCILED runtimeId=ARMY-GROUND-A7-RESTART-1 vehicle=4 personnel=12 credits="..tostring(reconciliation.reconciled))
  return true
end
...
local function prepareSite(site, templateGroup)
  ...
  site.campaignLossSettled = false
...
local function applyScenarioEffects(site)
  ...
  if not settleTestLoss(site) then return false end
...
local function attachArmyGroupCallbacks(site, armyGroup)
  if armyGroup.__omwGroundA7Callbacks then
    return
  end
  armyGroup.__omwGroundA7Callbacks = true
...
local function installBrigadeCallbacks(site)
  ...
  site.brigade.OnAfterStart = function(self, From, Event, To)
    ...
    SCHEDULER:New(nil, function()
      ...
      site.mission1:SetName("OMW_GND_A7_" .. site.id .. "_ROAD_APPROACH")
...
log("START testId=" .. TEST_ID .. " sites=" .. tostring(#sites)
  .. " campaignState=production-shaped-ground-nodes-48-20-22-with-restart-reconciliation")
...
if not verifyRestartReconciliation() then return end
