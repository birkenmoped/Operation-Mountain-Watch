...
Der Diagnostic-Code wurde aus dem eigenständigen Branch `agent/cree-bullseye-coordinate` auf Commit `b77a03840a0162c810a389b6a6581461607eb7e1` in den kombinierten Testbranch übernommen. Die fachliche und technische Scope-Grenze bleibt unverändert.
...
Im kombinierten Fuel-Telemetry-Test wird zuerst der gepinnte `Moose.lua`-Stand geladen. Danach können beide unabhängigen Bundles über getrennte `DO SCRIPT FILE`-Aktionen ausgeführt werden:

```text
mission/tests/aar-fuel-telemetry/dist/OMW_AAR_Fuel_Telemetry.lua
mission/tests/cree-bullseye-coordinate/dist/cree_bullseye_coordinate.lua
```

Erwarteter CREE-Logmarker:
