...
function fakeOpsZone:SetUnitCategories(value) self.unitCategories=value return self end
function fakeOpsZone:SetCaptureThreatlevel(value) self.captureThreatlevel=value return self end
...
local adapter = ThreatAdapter.New({
  ...
  priority=90, radiusM=1000, blueCoalition=2, redCoalition=1, updateSeconds=5,
  captureThreatlevel=0, captureNunits=1,
  zoneRadiusFactory=function(name, vec2, radiusM)
    createdZone={name=name, vec2=vec2, radiusM=radiusM}; return createdZone
  end,
  ...
  incidentIdFactory=function(_, incidentSequence)
    sequence=sequence+1; assertEqual(incidentSequence, sequence, "incident sequence")
  ...
  onThreatCleared=function(_, _, defeatedCoalition)
    ...
    assertEqual(defeatedCoalition, 1, "defeated coalition")
  end,
})
...
assertEqual(fakeOpsZone.captureThreatlevel, 0, "threat threshold")
assertEqual(fakeOpsZone.captureNunits, 1, "capture number")
...
local _, stoppedAgain = adapter:Stop(); assertFalse(stoppedAgain, "second stop idempotent")
