local TM02W2ELeaderProxyAdapter = {}
...
local function installSeparatedSpawn(spawner, rule, alias)
  ...
  if type(offset) ~= "table"
    ...
    error("TM02W2E leader proxy launch slot is unavailable for " .. tostring(alias))
  end
  ...
  function spawner:SpawnInZone(zone, randomize)
    if randomize ~= false then
      error("TM02W2E leader proxies require deterministic non-random launch positions")
    end
    if type(zone) ~= "table"
      or type(zone.GetVec2) ~= "function"
      or type(zone.IsVec2InZone) ~= "function" then
      error("TM02W2E source zone does not expose required vector APIs")
    end
    local center = zone:GetVec2()
    if type(center) ~= "table"
      or type(center.x) ~= "number"
      or type(center.y) ~= "number" then
      error("TM02W2E source zone center is unavailable")
    end
    local launchVec2 = {
      x = center.x + offset.x,
      y = center.y + offset.y,
    ...
    if zone:IsVec2InZone(launchVec2) ~= true then
      error("TM02W2E launch position is outside source zone for " .. tostring(alias))
    end
    ...
    env.info(
      "[OMW][TM02W2E] level=INFO event=red_task_launch_slot_applied"
...
local function deriveLeaderSpawner(rule, templateGroupName, alias)
  ...
  env.info(
    "[OMW][TM02W2E] level=INFO event=red_task_leader_template_derived"
...
function TM02W2ELeaderProxyAdapter.install(config)
  if type(config) ~= "table" or type(config.proxy) ~= "table" then
    error("TM02W2E proxy configuration is unavailable")
  end
  if type(SPAWN) ~= "table"
  ...
  env.info(
    "[OMW][TM02W2E] level=INFO event=red_task_leader_adapter_installed"
...
return TM02W2ELeaderProxyAdapter
