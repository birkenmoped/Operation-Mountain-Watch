-- Operation Mountain Watch - MOOSE-first incident-local Guard mission factory.
--
-- Creates an ONGUARD AUFTRAG at the validated local Guard materialization anchor.
-- The Guard does not receive a patrol route or proactive EngageTarget cycle; it is
-- local installation security while the QRF remains the mobile incident response.

local Factory = {}
local Instance = {}
Instance.__index = Instance

Factory.SchemaVersion = "OMW-FIRE-SUPPORT-STRATEGIC-RESUPPLY-GUARD-MISSION-FACTORY-3"
...
function Instance:Create(demand, context, legion)
  ...
  if type(mission.SetReturnToLegion) ~= "function" then fail("Guard AUFTRAG:SetReturnToLegion() is required") end
  ...
  self:_log(string.format(
    "created incident-local Guard mission demandId=%s siteId=%s localAnchor=true patrolRoute=false proactiveEngage=false returnToLegion=true attributes=%s properties=%s",
