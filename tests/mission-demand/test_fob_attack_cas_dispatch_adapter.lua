...
assertEqual(evidenceReason, "EVIDENCE_RECORDED_PENDING_MISSION_COMPLETION", "execution evidence remains non-terminal")
assertEqual(registry:Get(demand.id).status, MissionDemand.Status.ACTIVE, "shot evidence does not close active CAS demand")

mission:OnAfterSuccess("EXECUTING", "Success", "SUCCESS")
assertEqual(registry:Get(demand.id).status, MissionDemand.Status.SUCCESS, "AUFTRAG success plus execution evidence closes demand")
...
local _, closureRequested, closureReason = closureAdapter:RequestMissionClosure(closureDemand.id, "TACTICAL_CLOSURE")
...
assertEqual(registryClosure:Get(closureDemand.id).status, MissionDemand.Status.SUCCESS, "legacy success path remains supported when execution evidence is not required")
assertEqual(registryClosure:Get(closureDemand.id).result.closureRequested, true, "success records tactical closure request")
