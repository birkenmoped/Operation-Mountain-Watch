...
authoritative_for:
  - active OMW allocation of the documented 25-aircraft CH-47 pool across Bagram, FOB Salerno and FOB Shank
...
## 1. Binding OMW decision

Operation Mountain Watch adopts the following active allocation for the documented pool of 25 CH-47 aircraft distributed between Bagram, FOB Salerno and FOB Shank:

| Location | Assigned CH-47 inventory | Share of pool |
|---|---:|---:|
| Bagram Airfield | 13 | 52 percent |
| FOB Salerno | 6 | 24 percent |
| FOB Shank | 6 | 24 percent |
| **Total** | **25** | **100 percent** |
...
The `13/6/6` distribution is the binding campaign baseline. It is an evidence-based OMW reconstruction and must not be described as an explicitly published historical table.

## 2. Source and reconstruction basis

The historical source documents:

- 19 organic CH-47 plus 6 additional aircraft, for a total pool of 25;
- distribution of aircraft and crews between Bagram, FOB Salerno and FOB Shank;
- Bagram as the ISAF-wide general-support node;
...
The available source does not state the exact integer split. The project therefore combines organizational role, operational tasking and satellite evidence.

## 3. Satellite and tasking evidence
...
Bagram receives the remaining 13 aircraft. This is consistent with its role as the central general-support, maintenance, logistics and theater-wide tasking node.

```yaml
Bagram:
  assigned_inventory: 13
  allocation_basis:
    - central general-support role
    - larger logistics and maintenance capacity
    - remainder of documented 25-aircraft pool after two six-aircraft forward detachments
```
...
## 4. Assigned inventory is not visible ramp count

The following rule is binding:

```text
ASSIGNED_INVENTORY != AIRCRAFT_VISIBLE_ON_ONE_SATELLITE_IMAGE
```
...
## 5. Required inheritance by base documentation

The following base-specific values are binding and must be inherited by all manifests, implementation handoffs, runtime configurations and campaign-state inventories:

```text
Bagram CH-47 inventory: 13
FOB Salerno CH-47 inventory: 6
FOB Shank CH-47 inventory: 6
```

Any existing document that states a different active CH-47 inventory for one of these three locations is superseded for that subject by this decision and must be normalized when next edited.

This decision does not change the separately established Jalalabad CH-47 inventory.