-- Operation Mountain Watch - Production Base Acceptance 8.
...
-- before physical dispatch. A8 must never fall back to a direct-line helicopter route.

local TAG="[OMW][FSSR-PRODUCTION-BASE-A8]"
local TEST_ID="FSSR-PRODUCTION-BASE-ACCEPTANCE-8"
...
local EXECUTION_WATCHDOG_SEC=3600

local FlightPathNameContract=OMW_A8_FLIGHTPATH_NAME_CONTRACT
local HelicopterCorridor=OMW_A8_HELICOPTER_FLIGHTPATH_CORRIDOR
local CasTacticalCorridor=OMW_A8_HELICOPTER_CAS_TACTICAL_CORRIDOR
...
local function announce(kind,message,time)
  local text="[PRODUCTION BASE A8]["..kind.."] "..tostring(message)
  log(text)
...
local function isOurMission(mission)
  return mission and type(mission.GetName)=="function" and mission:GetName()=="OMW_FSSR_BASE_A8_CAS"
end
...
local function attachCommanderLifecycle(commander)
  ...
  function commander:OnAfterOpsOnMission(From,Event,To,OpsGroup,Mission)
    ...
    -- A8 must prove an earlier supported-element/no-contact release and owner-route
...
local function buildCommander()
  ...
  local commander=COMMANDER:New(coalition.side.BLUE,"OMW_FSSR_BASE_A8_C2")
...
local function buildBrigade(p)
  ...
  local brigade=BRIGADE:New(s.warehouseName,"BDE_FSSR_A8_"..SITE_ID)
  local guard=PLATOON:New(s.guardTemplateName,1,"PLT_FSSR_A8_GUARD_"..SITE_ID)
  local qrf=PLATOON:New(QRF_TEMPLATE,1,"PLT_FSSR_A8_QRF_"..SITE_ID)
...
local function resolveCasGeometry(demand,context)
  ...
  state.casZone=ZONE_RADIUS:New(
    "OMW_FSSR_BASE_A8_CAS_"..tostring(demand.demandId),
    coordinate:GetVec2(),
  ...
  return {
    ...
    configureMission=function(mission)
      mission:SetName("OMW_FSSR_BASE_A8_CAS")
    end,
  },nil
end
...
local function updateCasLifecycle()
  if state.passed or not state.casFlight then return end
  ...
  if state.casGroup and state.casInitialAlive and type(state.casGroup.CountAliveUnits)=="function" then
    ...
    if alive<state.casInitialAlive and not state.failed then
      fail("CAS_ASSET_LOSS initialAlive="..tostring(state.casInitialAlive).." alive="..tostring(alive))
    end
  end

  -- MOOSE is the on-station authority. AUFTRAG:IsExecuting() means the first
  -- OPSGROUP has reached the mission execution waypoint and is executing the
  -- mission task. A geometric zone check was too strict in A7 and prevented the
  -- release monitor from ever starting although MOOSE had begun PATROLZONE.
  if state.casMission and type(state.casMission.IsExecuting)=="function"
      and state.casMission:IsExecuting() and not state.casOnStation then
    state.casOnStation=true
    state.casExecutingSince=timer.getAbsTime()
    log("CAS_ON_STATION source=AUFTRAG_IsExecuting provider="..tostring(state.selectedAirwingAlias)
      .." squadron="..tostring(state.selectedSquadron))
  end

  if not state.casOnStation or state.recoveryRequested then return end

  local eligible,total,sensorReady=casDetectedEligibleGroups()
  if not sensorReady then
    if not state.sensorReady then
      log("CAS_SENSOR_WAIT source=FLIGHTGROUP_GetDetectedGroups")
    end
    return
  end
  ...
  elseif count==0 and not state.casNoContactSince then
    state.casNoContactSince=timer.getAbsTime()
    log(string.format("CAS_SENSOR_REPORT detectedTotal=%d eligible=0 qualification=PENDING_%ds",
      total,CAS_NO_CONTACT_STABLE_SEC))
  end
  ...
  if state.supportedElementClear then
    log("CAS_SUPPORTED_ELEMENT_CLEAR livingIncidentGroups=0")
  end

  if state.supportedElementClear and state.casNoContactReported then
    local changed,reason=state.casHandle:Cancel()
    if changed~=true then
      if not state.failed then fail("CAS_CONTROLLED_RELEASE_FAILED "..tostring(reason)) end
      return
    end
...
local function evaluate()
  ...
  if not state.failed
    ...
    state.passed=true
    announce("PASS",
      "Joyce attack -> production incident/QRF plus Base CAS demand -> MOOSE-selected attack-helicopter provider/asset -> owner route -> MOOSE mission execution -> own detection/no-contact release -> reverse owner route -> physical landing -> LEGION asset return.",
      45)
    return
  end

  -- Watchdog is diagnostic only. It must never stop the safety/lifecycle monitor.
  -- A7 used a terminal timeout and therefore disabled release monitoring while the
  -- aircraft was still outbound. A8 keeps monitoring until recovery or user stop.
  if state.startedAt and not state.watchdogWarned
      and timer.getTime()-state.startedAt>EXECUTION_WATCHDOG_SEC then
    state.watchdogWarned=true
    announce("WARN","Execution watchdog elapsed; lifecycle monitoring continues. qrfEngage="
      ..tostring(state.qrfEngageObserved)
      ...
      .." recovery="..tostring(state.recoveryRequested),20)
  end
end
...
local function start()
  ...
  announce("READY",
    "Joyce Base runtime active. MOOSE COMMANDER owns provider/asset selection. A8 requires PATROLZONE attack-helicopter capability and binds an owner route only after MOOSE selects the provider.",
    20)
