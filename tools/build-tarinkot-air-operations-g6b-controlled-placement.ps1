...
$sourceFile = Join-Path $sourceDir '03-tarinkot-g6b-controlled-placement.lua'
...
foreach ($entry in $forbiddenPatterns.GetEnumerator()) {
    if ($sourceText -match $entry.Value) {
        throw "G6B controlled-placement guard rejected source: $($entry.Key) matched pattern $($entry.Value)"
    }
}
...
foreach ($entry in $requiredPatterns.GetEnumerator()) {
    if ($sourceText -notmatch $entry.Value) {
        throw "G6B controlled-placement guard requires: $($entry.Key) pattern $($entry.Value)"
    }
}
...
$variants = @(
    [ordered]@{
        ...
        OutputName = 'OMW_AirOps_Tarinkot_G6B_AH64_Placement.lua'
        Requests = @(
            [ordered]@{
                Alias = 'TKOT_G6B_AH64_PAIR'
                ExpectedUnits = 2
                Spots = @(0, 25)
            }
        )
    ...
    [ordered]@{
        ...
        ModelRadiusM = '10.020'
        ExpectedTotalUnits = 2
        OutputName = 'OMW_AirOps_Tarinkot_G6B_UH60_Placement.lua'
        Requests = @(
            [ordered]@{
                Alias = 'TKOT_G6B_UH60_LEAD'
                ExpectedUnits = 1
                Spots = @(13)
            },
            [ordered]@{
                Alias = 'TKOT_G6B_UH60_SUPPORT'
                ExpectedUnits = 1
                Spots = @(22)
            }
        )
    ...
    [ordered]@{
        ...
        OutputName = 'OMW_AirOps_Tarinkot_G6B_CH47_Placement.lua'
        Requests = @(
            [ordered]@{
                Alias = 'TKOT_G6B_CH47_SINGLE'
                ExpectedUnits = 1
                Spots = @(14)
            }
        )
...
$builderVersion = 'TKOT-G6B-CONTROLLED-PLACEMENT-1'
$commit = 'UNKNOWN'
try {
...
-- Builder: tools/build-tarinkot-air-operations-g6b-controlled-placement.ps1
-- BuilderVersion: $builderVersion
-- GitCommit: $commit
-- GeneratedUtc: $generatedUtc
-- TestGate: G6B_$($variant.Family)_CONTROLLED_PLACEMENT
...
local OMW_TKOT_G6B_CONFIG = {
  Family = "$($variant.Family)",
  TemplateGroup = "$($variant.TemplateGroup)",
  ExpectedType = "$($variant.ExpectedType)",
  ModelRadiusM = $($variant.ModelRadiusM),
  ExpectedTotalUnits = $([int]$variant.ExpectedTotalUnits),
  SpawnRequests = {
$($requestLines -join "`r`n")
  }
}

-- BEGIN SOURCE: 03-tarinkot-g6b-controlled-placement.lua
"@

    $footer = @"

-- END SOURCE: 03-tarinkot-g6b-controlled-placement.lua
"@

    $outputFile = Join-Path $distDir $variant.OutputName
...
foreach ($item in $built) {
    Write-Host "BuiltFamily: $($item.Family)"
    Write-Host "Built: $($item.Path)"
...
Write-Host "BundlesBuilt: $($built.Count)"
